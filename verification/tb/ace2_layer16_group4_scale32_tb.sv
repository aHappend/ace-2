...
module ace2_layer16_group4_scale32_tb;
    `include "ace2_layer16_group4_scale32/ace2_layer16_group4_scale32_constants.svh"

    localparam integer Q_GROUPS = MODEL_HIDDEN_SIZE / 4;
    ...
    logic d_in_ready_o;
    logic signed [31:0] d_attention_source_s8_i = '0;
    logic signed [31:0] d_down_source_s8_i = '0;
    ...
    logic signed [31:0] d_attention_group_s8_o;
    logic signed [31:0] d_down_group_s8_o;
    logic signed [31:0] d_sum_s8_o;
    logic [3:0] d_attention_saturation_o;
    logic [3:0] d_down_saturation_o;
    logic [3:0] d_sum_saturation_o;
    logic d_descriptor_error_o;
    logic d_numeric_overflow_o;

    ace2_group4_scale32_quant_sum_core directed_dut (
        .clk_i,
        .rst_ni,
        .clear_i,
    ...
    logic group_ready_o;
    logic signed [31:0] attention_source_s8_i = '0;
    logic signed [31:0] down_source_s8_i = '0;
    ...
    logic signed [15:0] q_weight_s4_i = '0;
    ...
    logic [7:0] group_debug_index_o;
    logic signed [31:0] group_debug_attention_s8_o;
    logic signed [31:0] group_debug_down_s8_o;
    logic signed [31:0] group_debug_sum_s8_o;
    logic [3:0] group_debug_attention_saturation_o;
    logic [3:0] group_debug_down_saturation_o;
    logic [3:0] group_debug_sum_saturation_o;
    ...
    ace2_layer16_group4_rmsnorm_q_core dut (
        .clk_i,
        .rst_ni,
        .clear_i,
        .start_valid_i,
    ...
    logic [31:0] attention_source_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] down_source_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] attention_scale_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] down_scale_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] expected_attention_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] expected_down_mem [0:MODEL_GROUP_COUNT-1];
    logic [31:0] expected_sum_mem [0:MODEL_GROUP_COUNT-1];
    logic [255:0] gain_mem [0:RMS_BEATS-1];
    logic [127:0] expected_rms_mem [0:RMS_BEATS-1];
    logic [15:0] q_weight_mem [0:Q_WEIGHT_WORDS-1];
    ...
    task automatic directed_case(
        input string name,
        input logic signed [31:0] attention_source,
        input logic signed [31:0] down_source,
        ...
        input logic signed [31:0] expected_attention,
        input logic signed [31:0] expected_down,
        input logic signed [31:0] expected_sum,
        input logic [3:0] expected_attention_saturation,
        input logic [3:0] expected_down_saturation,
        input logic [3:0] expected_sum_saturation,
        input logic expected_descriptor_error
    );
        logic signed [31:0] held_attention;
        logic signed [31:0] held_down;
        logic signed [31:0] held_sum;
        integer timeout;
        begin
            timeout = 0;
    ...
    always @(posedge clk_i) begin
        ...
        if (rst_ni && !clear_i) begin
            if ($isunknown({
                d_in_ready_o, d_out_valid_o, d_attention_group_s8_o,
                d_down_group_s8_o, d_sum_s8_o, d_attention_saturation_o,
                d_down_saturation_o, d_sum_saturation_o,
                d_descriptor_error_o, d_numeric_overflow_o,
                start_ready_o, group_ready_o, gain_ready_o,
                q_row_start_ready_o, q_weight_ready_o, q_meta_ready_o,
                q_out_valid_o, q_out_s8_o, q_accumulator_s32_o,
                q_row_tag_o, q_saturation_o, q_accumulator_overflow_o,
                group_debug_valid_o, group_debug_index_o,
                group_debug_attention_s8_o, group_debug_down_s8_o,
                group_debug_sum_s8_o, group_debug_attention_saturation_o,
                group_debug_down_saturation_o, group_debug_sum_saturation_o,
                norm_debug_valid_o, norm_debug_index_o, norm_debug_s8_o,
                done_valid_o, done_tag_o, attention_saturation_count_o,
                down_saturation_count_o, sum_saturation_count_o,
                rms_saturation_o, q_saturation_count_o,
                descriptor_error_o, numeric_overflow_o
            }))
                $fatal(1, "X/Z detected on focused RTL outputs at cycle %0d", cycle_count);
        end
        if (model_check_active && group_debug_valid_o) begin
            if (group_debug_index_o !== group_debug_count[7:0] ||
                ...
                group_debug_attention_saturation_o !== 4'b0000 ||
                group_debug_down_saturation_o !== 4'b0000 ||
                group_debug_sum_saturation_o !== 4'b0000)
    ...
    initial begin
        $readmemh("verification/generated/ace2_layer16_group4_scale32/attention_source_s8.hex", attention_source_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/down_source_s8.hex", down_source_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/attention_group_scale32.hex", attention_scale_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/down_group_scale32.hex", down_scale_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/expected_attention_group_s8.hex", expected_attention_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/expected_down_group_s8.hex", expected_down_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/expected_sum_s8.hex", expected_sum_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/rms_gain_s16_q8.hex", gain_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/expected_rms_s8.hex", expected_rms_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_weight_s4.hex", q_weight_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_multiplier_s32.hex", q_multiplier_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_shift_u6.hex", q_shift_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_expected_accumulator_s32.hex", q_expected_acc_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_expected_s8.hex", q_expected_mem);
        $readmemh("verification/generated/ace2_layer16_group4_scale32/q_expected_saturation.hex", q_expected_saturation_mem);
        ...
        @(negedge clk_i);
        d_attention_source_s8_i = 32'h01020304;
        d_down_source_s8_i = 32'h05060708;
        ...
        repeat (2) begin
            @(posedge clk_i);
            #1;
            if (!done_valid_o || done_tag_o !== 16'h1740)
        ...
        $display(
            "ACE2_LAYER16_GROUP4_SCALE32_RTL_PASS directed=%0d groups=%0d rms_beats=%0d q_outputs=%0d q_saturations=%0d source_stalls=%0d gain_stalls=%0d weight_stalls=%0d output_stalls=%0d cycles=%0d xz_clean=1 reset_recovery=1 clear_recovery=1",
