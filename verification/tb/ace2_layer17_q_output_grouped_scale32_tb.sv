...
module ace2_layer17_q_output_grouped_scale32_tb;
    `include "ace2_layer17_q_output_grouped_scale32/ace2_layer17_q_output_grouped_scale32_constants.svh"
    ...
    reg [31:0] accumulator_mem [0:MODEL_Q_OUTPUTS-1];
    reg [31:0] multiplier_mem [0:MODEL_Q_OUTPUTS-1];
    reg [7:0] shift_mem [0:MODEL_Q_OUTPUTS-1];
    reg [31:0] scale_mem [0:MODEL_Q_OUTPUT_GROUP_COUNT-1];
    reg [7:0] expected_mem [0:MODEL_Q_OUTPUTS-1];
    reg [7:0] saturation_mem [0:MODEL_Q_OUTPUTS-1];
    ...
    task apply_reset;
        begin
            ...
            if (!in_ready || out_valid) begin
                $display("LAYER17_Q_OUTPUT_RESET_RECOVERY_FAIL ready=%b valid=%b", in_ready, out_valid);
                failures = failures + 1;
            end
        end
    endtask
    ...
    task produce_only;
        ...
        begin
            ...
            if (!out_valid) begin
                $display("LAYER17_Q_OUTPUT_PRODUCE_TIMEOUT");
                failures = failures + 1;
            end
        end
    endtask
        ...
        begin
            ...
            for (stall_index = 0; stall_index < output_stall_cycles; stall_index = stall_index + 1) begin
                ...
                if (!out_valid || q !== held_q || output_scale32 !== held_scale ||
                    ...
                    $display("LAYER17_Q_OUTPUT_BACKPRESSURE_STABILITY_FAIL channel=%0d", selected_channel);
                    failures = failures + 1;
                end
            end
            if (q !== expected_q || saturation !== expected_saturation ||
                ...
                $display("LAYER17_Q_OUTPUT_MISMATCH channel=%0d q=%02x expected=%02x sat=%0d expected_sat=%0d scale=%08x expected_scale=%08x last=%0d expected_last=%0d",
                         selected_channel, q, expected_q, saturation, expected_saturation,
    ...
    task clear_recovery_probe;
        begin
            ...
            repeat (2) @(posedge clk);
            if (!in_ready || out_valid) begin
                $display("LAYER17_Q_OUTPUT_CLEAR_RECOVERY_FAIL ready=%b valid=%b", in_ready, out_valid);
                failures = failures + 1;
            end
        end
    endtask
    ...
    task async_reset_recovery_probe;
        begin
            ...
            #1;
            if (out_valid) begin
                $display("LAYER17_Q_OUTPUT_ASYNC_RESET_DID_NOT_CLEAR_VALID");
                failures = failures + 1;
            end
            repeat (2) @(posedge clk);
            @(negedge clk);
            ...
            repeat (2) @(posedge clk);
            if (!in_ready || out_valid) begin
                $display("LAYER17_Q_OUTPUT_ASYNC_RESET_RECOVERY_FAIL ready=%b valid=%b", in_ready, out_valid);
                failures = failures + 1;
            end
        end
    endtask
    ...
    initial begin
        ...
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_accumulator_s32.hex", accumulator_mem);
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_multiplier_s32.hex", multiplier_mem);
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_shift_u6.hex", shift_mem);
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_output_group_scale32.hex", scale_mem);
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_expected_s8.hex", expected_mem);
        $readmemh("verification/generated/ace2_layer17_q_output_grouped_scale32/q_expected_saturation.hex", saturation_mem);
        ...
        for (index = 0; index < MODEL_Q_OUTPUTS; index = index + 1) begin
            send_and_check(
                accumulator_mem[index],
                multiplier_mem[index],
                shift_mem[index][5:0],
                scale_mem[index / MODEL_Q_OUTPUT_GROUP_SIZE],
                index[9:0],
                index == MODEL_Q_OUTPUTS - 1,
                expected_mem[index],
            ...
            if ((index % MODEL_Q_OUTPUT_GROUP_SIZE) == 0)
                group_count = group_count + 1;
            saturation_count = saturation_count + saturation_mem[index][0];
        end

        if (output_count != MODEL_Q_OUTPUTS ||
            group_count != MODEL_Q_OUTPUT_GROUP_COUNT ||
            saturation_count != MODEL_Q_SATURATION_COUNT || xz_count != 0)
            failures = failures + 1;

        if (failures == 0) begin
            $display("ACE2_LAYER17_Q_OUTPUT_GROUPED_SCALE32_RTL_PASS directed=%0d outputs=%0d groups=%0d saturations=%0d input_stalls=%0d output_stalls=%0d cycles=%0d xz_clean=1 reset_recovery=1 clear_recovery=1",
                     directed_count, output_count, group_count, saturation_count,
                     input_stalls, output_stalls, cycle_count);
        end else begin
            $display("ACE2_LAYER17_Q_OUTPUT_GROUPED_SCALE32_RTL_FAIL failures=%0d xz=%0d", failures, xz_count);
            $fatal(1);
        end
        $finish;
    end
