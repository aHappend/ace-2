#!/usr/bin/env python3
"""Synthetic-only verifier for the isolated-root Base G16 V4 compatibility package."""
...
ACTION_ROOT_ID = "qk-gbfp8-g16-base-cardinality-one-authority-static-v4-action-root"
PACKAGE_REL = "reference/QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V4_PACKAGE.json"
SIDECAR_REL = PACKAGE_REL + ".sha256"
PACKAGE_PATH = ROOT / PACKAGE_REL
SIDECAR_PATH = ROOT / SIDECAR_REL
VERIFIER_PATH = Path(__file__).resolve()
CONTROLLER_REL = "reference/qk_gbfp8_g16_base_cardinality_one_authority_controller_static_v4.py"
...
AUTHORITY_REVIEW_MISSION_ID = "4ae7875505a9"
AUTHORITY_REVIEW_DIRECTORY = Path(
    "/home/argustest/.argus-skill-ace2/projects/s-c8ae985b/handoffs/4ae7875505a9"
)
AUTHORITY_REVIEW_MISSION_PATH = AUTHORITY_REVIEW_DIRECTORY / "mission.json"
AUTHORITY_REVIEW_MISSION_SHA256 = "867465a2ffe499258e1240b301c57697ada3d2c7db3ac20d9826edcd2d1985ab"
...
G16_EVALUATOR_REL = "reference/qk_gbfp8_g16_head64_successor_diagnostic_static_v4.py"
...
G16_PACKAGE_SHA256 = "2d7f58d0b520babed1e78e6c2535169984fae461157cd108e7d15568ecca3e2a"
G16_CONTRACT_SHA256 = "2c4ec09a3fc460e310c5c812c1069f3a3019e951234ac53e1ce3f77d5cb664fa"
G16_SCHEMA_SHA256 = "8d512af26bf39593236098f850ae5477ae13343ffbdeac4fbb207b1b8fd71840"
G16_EVALUATOR_SHA256 = "2c51e985f3aabbfea85767fa2e26c91fb9670e25e77cbf0169185cf2bef48682"
...
G16_SIDECAR_SHA256 = "16533c28b27cc02b53cca7854516fbb5be7a93c606c8522a41dd39fcb5210944"
ACCEPTED_G16_PACKAGE_SHA256 = "33c42fe340867f5b89d451339ca2d552f0bfec3309d8a6103066c171d7806513"

AUTHORITY_PACKAGE_ID = "QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V4"
AUTHORITY_ID = "qk-gbfp8-g16-base-cardinality-one-v4"
AUTHORITY_ROOT_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v4-authority"
...
RESULT_ROOT_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v4"
...
PACKAGE_SCHEMA_REL = "reference/QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V4_PACKAGE_SCHEMA.json"
CREDENTIAL_SCHEMA_REL = "reference/QK_GBFP8_G16_BASE_CARDINALITY_ONE_CREDENTIAL_V2_SCHEMA.json"
TERMINAL_SCHEMA_REL = "reference/QK_GBFP8_G16_BASE_FIRST_TERMINAL_V4_SCHEMA.json"
...
BASE_INVOCATION_SHA256 = "8ba4d14d206be2d5e93739caf67dd756dd19039c130bea9c5ee2b31f7b6fa9c9"
...
FIRST_TERMINAL_SCHEMA_ID = "QK_GBFP8_G16_BASE_FIRST_TERMINAL_V4"
    ...
    schema_bindings = {
        "authority_package": {
            **repo_binding(PACKAGE_SCHEMA_REL, sha256_file(ROOT / PACKAGE_SCHEMA_REL)),
            "schema_id": "QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V4_PACKAGE_SCHEMA",
        },
        "credential": {
        ...
        "first_terminal": {
            **repo_binding(TERMINAL_SCHEMA_REL, sha256_file(ROOT / TERMINAL_SCHEMA_REL)),
            "schema_id": "QK_GBFP8_G16_BASE_FIRST_TERMINAL_V4_SCHEMA",
        },
    }
    return {
        "action_root_projection": {
            ...
            "evaluator_argv_compatibility_revision": "V4 evaluator and create-only namespaces only",
            "evaluator_argv_exactly_accepted": False,
            "namespace_separation": "All V4 compatibility evaluator paths resolve beneath this distinct V4 action root, never beneath the consumed V3 action root or closed repository-root V1 action.",
        ...
        "artifact_kind": "qk_gbfp8_g16_base_cardinality_one_execution_authority_static_v4_action_root_freeze",
        "authority_controller": {
            ...
            "controller_id": "QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_CONTROLLER_STATIC_V4",
        ...
        "static_verification": {
            "coverage": [
                ...
                "isolated V4 action-root projection and closed V1/V2 plus consumed V3 immutability",
                "authoritative big-endian count, length, rank, dimension, and payload framing",
                "valid 25-record and boundary 1/256 synthetic containers with zero/257 rejection",
                "truncation, trailing-byte, duplicate-name, little-endian, identity, and checksum fault rejection",
        ...
        "v4_compatibility_runtime": {
            **compatibility_runtime_bindings(),
            "framing": {
                "count_and_length_fields": "big-endian >I",
                "dimension_and_payload_length_fields": "big-endian >Q",
                "magic": "ACE2-C02-TENSORS-V1\n",
                "rank_fields": "big-endian >H",
                "record_checksum_semantics": "unchanged V3 tensor_record_sha256",
            },
        },
    }
...
def historical_json_compatibility_checks(package: dict[str, Any]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="g16-v4-json-") as temporary:
        root = Path(temporary)
        ...
        require(load_canonical_json(canonical) == semantic, "new V4 canonical load")
        checks += 1
    ...
    credential = {
        ...
        "irreversible_action_id": "ace2:g16-base:v4:synthetic",
        "lane_label": "Base",
        "schema_id": CREDENTIAL_SCHEMA_ID,
    }
...
def review_binding_checks(package: dict[str, Any]) -> int:
    ...
    valid = {
        ...
        "review": {
            "next_action": "",
            "operator_question": "",
            "reason": f"Fresh-L2 accepts runtime package {package_sha256}. No V4 authority exists.",
            "status": "done",
        },
        "round": 9999,
        "schema_version": 2,
...
def mutation_checks(package: dict[str, Any]) -> int:
    ...
    mutations = [
        ...
        (("v4_compatibility_runtime", "evaluator", "sha256"), "0" * 64),
        (("v4_compatibility_runtime", "framing", "rank_fields"), "little-endian <H"),
...
def synthetic_tensor_record_sha256(dtype: str, shape: list[int], payload: bytes) -> str:
    frame = bytearray()
    encoded_dtype = dtype.encode("utf-8")
    frame.extend(len(encoded_dtype).to_bytes(4, "little"))
    frame.extend(encoded_dtype)
    frame.extend(len(shape).to_bytes(4, "little"))
    for dimension in shape:
        frame.extend(dimension.to_bytes(8, "little"))
    frame.extend(len(payload).to_bytes(8, "little"))
...
def synthetic_bundle_bytes(
    ...
    *,
    ...
    require(byte_order in {"big", "little"}, "synthetic byte order")
    prefix = ">" if byte_order == "big" else "<"
    payload = bytearray(TENSOR_BUNDLE_MAGIC)
    payload.extend(struct.pack(prefix + "I", len(records) if count_override is None else count_override))
    for name, dtype, shape, record_payload in records:
        name_raw = name.encode("utf-8")
        dtype_raw = dtype.encode("ascii")
        payload.extend(struct.pack(prefix + "I", len(name_raw)))
        payload.extend(name_raw)
        payload.extend(struct.pack(prefix + "I", len(dtype_raw)))
        payload.extend(dtype_raw)
        payload.extend(struct.pack(prefix + "H", len(shape)))
        for dimension in shape:
            payload.extend(struct.pack(prefix + "Q", dimension))
        payload.extend(struct.pack(prefix + "Q", len(record_payload)))
        payload.extend(record_payload)
    return bytes(payload)


def synthetic_parse_tensor_bundle(payload: bytes) -> dict[str, dict[str, Any]]:
    require(type(payload) is bytes, "synthetic bundle bytes")
    cursor = 0

    def read(size: int) -> bytes:
        nonlocal cursor
        require(type(size) is int and size >= 0 and cursor + size <= len(payload), "truncated synthetic bundle")
        value = payload[cursor : cursor + size]
        cursor += size
        return value

    require(read(len(TENSOR_BUNDLE_MAGIC)) == TENSOR_BUNDLE_MAGIC, "synthetic tensor magic")
    count = struct.unpack(">I", read(4))[0]
    require(1 <= count <= 256, "synthetic tensor record count")
    records: dict[str, dict[str, Any]] = {}
    for _ in range(count):
        name_size = struct.unpack(">I", read(4))[0]
        require(1 <= name_size <= 1024, "synthetic tensor name size")
        name = read(name_size).decode("utf-8")
        require(name not in records, "synthetic duplicate tensor name")
        dtype_size = struct.unpack(">I", read(4))[0]
        require(1 <= dtype_size <= 64, "synthetic tensor dtype size")
        dtype = read(dtype_size).decode("ascii")
        rank = struct.unpack(">H", read(2))[0]
        require(1 <= rank <= 8, "synthetic tensor rank")
        shape = tuple(struct.unpack(">Q", read(8))[0] for _ in range(rank))
        require(all(1 <= dimension <= (1 << 20) for dimension in shape), "synthetic tensor shape")
        payload_size = struct.unpack(">Q", read(8))[0]
        require(payload_size <= (1 << 31), "synthetic tensor payload size")
        record_payload = read(payload_size)
        records[name] = {
            "dtype": dtype,
            "payload": record_payload,
            "sha256": synthetic_tensor_record_sha256(dtype, list(shape), record_payload),
            "shape": list(shape),
        }
    require(cursor == len(payload), "synthetic trailing tensor bytes")
    return records


def framing_compatibility_checks() -> int:
    def record(index: int, *, shape: tuple[int, ...] = (1,)) -> tuple[str, str, tuple[int, ...], bytes]:
        return (f"synthetic.record.{index:03d}", "torch.bfloat16", shape, b"\x80\x3f" * math.prod(shape))

    checks = 0
    for count in (1, 25, 256):
        records = [record(index) for index in range(count)]
        parsed = synthetic_parse_tensor_bundle(synthetic_bundle_bytes(records))
        require(len(parsed) == count, f"valid big-endian record count {count}")
        checks += 1
    for count in (0, 257):
        expect_reject(synthetic_parse_tensor_bundle, synthetic_bundle_bytes([], count_override=count))
        checks += 1

    first = record(0, shape=(1, 256))
    valid = synthetic_bundle_bytes([first])
    name_size = len(first[0].encode("utf-8"))
    dtype_size = len(first[1].encode("ascii"))
    magic_end = len(TENSOR_BUNDLE_MAGIC)
    name_end = magic_end + 4 + 4 + name_size
    dtype_end = name_end + 4 + dtype_size
    rank_end = dtype_end + 2
    dimension_end = rank_end + 16
    payload_length_end = dimension_end + 8
    truncations = (
        valid[: magic_end - 1],
        valid[: magic_end + 3],
        valid[: name_end - 1],
        valid[: dtype_end - 1],
        valid[: rank_end - 1],
        valid[: dimension_end - 1],
        valid[: payload_length_end + len(first[3]) - 1],
    )
    for truncated in truncations:
        expect_reject(synthetic_parse_tensor_bundle, truncated)
        checks += 1
    expect_reject(synthetic_parse_tensor_bundle, valid + b"\x00")
    checks += 1
    duplicate = [record(0), record(0)]
    expect_reject(synthetic_parse_tensor_bundle, synthetic_bundle_bytes(duplicate))
    checks += 1
    interpreted = synthetic_parse_tensor_bundle(valid)[first[0]]
    require(interpreted["shape"] == [1, 256], "big-endian rank and dimension interpretation")
    checks += 1
    for count in (1, 25):
        expect_reject(
            synthetic_parse_tensor_bundle,
            synthetic_bundle_bytes([record(index) for index in range(count)], byte_order="little"),
        )
        checks += 1

    selected_records, bindings = selected_record_fixture()
    mutations: list[Callable[[dict[str, Any], dict[str, Any]], None]] = [
        lambda records, expected: expected[SELECTED_ROLES[0]].__setitem__("tensor_name", "synthetic.wrong-name"),
        lambda records, expected: records[expected[SELECTED_ROLES[0]]["tensor_name"]].__setitem__("dtype", "torch.float16"),
        lambda records, expected: records[expected[SELECTED_ROLES[0]]["tensor_name"]].__setitem__("shape", [2, 1]),
        lambda records, expected: expected[SELECTED_ROLES[0]].__setitem__("sha256", "0" * 64),
        lambda records, expected: expected[SELECTED_ROLES[0]].__setitem__("sha256", "0" * 63),
    ]
    for mutate in mutations:
        record_mutation = copy.deepcopy(selected_records)
        binding_mutation = copy.deepcopy(bindings)
        mutate(record_mutation, binding_mutation)
        expect_reject(validate_synthetic_selected_records, record_mutation, binding_mutation)
        checks += 1
    return checks
...
def verify_static_bindings(package: dict[str, Any]) -> int:
    ...
    require(package["v4_compatibility_runtime"] == expected_runtime, "V4 compatibility runtime closure")
    checks += 1
...
def normalized_evaluator_ast(source: str) -> str:
    ...
    for index, node in enumerate(tree.body):
        if index == 0 and isinstance(node, ast.Expr) and isinstance(node.value, ast.Constant) and type(node.value.value) is str:
            continue
        if isinstance(node, ast.FunctionDef) and node.name == "read_tensor_bundle":
            continue
...
def evaluator_compatibility_surface_checks() -> int:
    v4_source = (ROOT / G16_EVALUATOR_REL).read_text(encoding="utf-8")
    ...
    require(normalized_evaluator_ast(v4_source) == normalized_evaluator_ast(v3_source), "V4 evaluator non-framing drift")
    v4_tree = ast.parse(v4_source)
    v3_tree = ast.parse(v3_source)
    reader_nodes = [node for node in ast.walk(v4_tree) if isinstance(node, ast.FunctionDef) and node.name == "read_tensor_bundle"]
    ...
    reader_source = ast.get_source_segment(v4_source, reader_nodes[0]) or ""
    old_reader_source = ast.get_source_segment(v3_source, old_reader_nodes[0]) or ""
    require(reader_source.count('struct.unpack(">I", read_exact(handle, 4))[0]') == 3, "V4 big-endian count/length fields")
    require(reader_source.count('struct.unpack(">H", read_exact(handle, 2))[0]') == 1, "V4 big-endian rank field")
    require(reader_source.count('struct.unpack(">Q", read_exact(handle, 8))[0]') == 2, "V4 big-endian dimension/payload fields")
    require('"little"' not in reader_source and "'little'" not in reader_source, "V4 reader little-endian residue")
    require(old_reader_source.count('"little"') >= 6, "V3 little-endian root-cause surface")
    finiteness = [
        node for node in ast.walk(v4_tree) if isinstance(node, ast.FunctionDef) and node.name == "evaluate_selected_records_from_reference"
    ]
    require(len(finiteness) == 1, "reference evaluation function cardinality")
    finiteness_source = ast.get_source_segment(v4_source, finiteness[0]) or ""
    ...
    require(
        function_ast(v4_tree, "tensor_record_sha256") == function_ast(v3_tree, "tensor_record_sha256"),
        "per-record checksum semantics preserved",
    )
    return 9
...
def verify_no_execution_surface() -> int:
    ...
    tree = ast.parse(source)
    forbidden_modules = {"cocotb", "importlib", "numpy", "onnxruntime", "subprocess", "tensorflow", "torch", "transformers"}
    forbidden_calls = {"Popen", "eval", "exec", "from_pretrained", "generate", "popen", "run", "system"}
    imported: set[str] = set()
    called: set[str] = set()
    ...
    return len(forbidden_modules) + len(forbidden_calls)
...
def main() -> int:
    ...
    print("QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V4=PASS")
    ...
    print(f"CLOSED_V1_V2_V3_IMMUTABILITY_CHECKS={closed_v1_checks + closed_v2_checks + consumed_v3_checks}")
    ...
    print("PROJECTED_SEALED_TENSOR_SLOT=ABSENT EVALUATOR_ARGV_COMPATIBILITY_REVISION=V4")
