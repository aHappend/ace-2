#!/usr/bin/env python3
"""Deterministic G16 Q/K diagnostic V4 reference for a later authorized run.
...
OUTPUT_ROOT_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v4"
...
def read_tensor_bundle(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("rb") as handle:
        ...
        for _ in range(count):
            name_size = struct.unpack(">I", read_exact(handle, 4))[0]
            ...
            dtype_size = struct.unpack(">I", read_exact(handle, 4))[0]
            require(1 <= dtype_size <= 64, "tensor dtype size")
            dtype = read_exact(handle, dtype_size).decode("ascii")
            rank = struct.unpack(">H", read_exact(handle, 2))[0]
