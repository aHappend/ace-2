...
def _owner_claim(action_id: str) -> dict[str, Any]:
    return sealed({
        "action_id": action_id,
        "action_retired": True,
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v20_exactly_once_owner_claim",
...
def _authority(bindings: dict[str, Any]) -> dict[str, Any]:
    return sealed({
        ...
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v20_exactly_once_authority",
        "binding_record_count": bindings["binding_record_count"],
...
def _credential(action_id: str, authority: dict[str, Any]) -> dict[str, Any]:
    ...
    return sealed({
        "action_id": action_id,
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v20_exactly_once_credential",
...
def _ledger(bindings: dict[str, Any], authority: dict[str, Any], credential: dict[str, Any]) -> dict[str, Any]:
    return sealed({
        "action_id": bindings["action_id"],
        "action_retired": True,
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v20_exactly_once_consumed_ledger",
        "authority_sha256": authority["authority_sha256"],
...
def _terminal(
    ...
    *,
    ...
    return sealed({
        "action_id": action_id,
        "action_retired": True,
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v20_exactly_once_first_terminal",
...
def _validate_terminal(record: dict[str, Any], validate: Callable[[dict[str, Any]], None] | None) -> None:
    verify_self_hash(record, "first_terminal_sha256")
    if record["action_retired"] is not True or record["first_record_immutable"] is not True:
        raise LifecycleError("terminal retirement invariant")
    if record["retry_replay_resume_repair_replacement_permitted"] is not False:
        raise LifecycleError("terminal retry invariant")
    if validate is not None:
        validate(record)


def _publish_preconsumption_terminal(
    paths: RuntimePaths,
    validate: Callable[[dict[str, Any]], None] | None,
    record: dict[str, Any],
    faults: frozenset[str],
) -> tuple[str | None, bool]:
    _validate_terminal(record, validate)
    try:
...
def _publish_postconsumption_terminal(
    ...
    try:
        ...
        _validate_terminal(orphan_record, validate)
        try:
...
def _prepare_runtime_namespace(paths: RuntimePaths) -> None:
...
def _other_lifecycle_exists(paths: RuntimePaths) -> bool:
    return any(os.path.lexists(path) for path in paths if path != paths.owner)


def _claim_owner(paths: RuntimePaths, action_id: str, faults: frozenset[str]) -> dict[str, Any] | None:
    _prepare_runtime_namespace(paths)
    if _other_lifecycle_exists(paths):
        return None
    claim = _owner_claim(action_id)
    verify_self_hash(claim, "owner_claim_sha256")
    try:
        durable_create(paths.owner, compact_bytes(claim), fault="OWNER_WRITE" in faults)
    except FileExistsError:
        return None
    return claim


def _preflight_stage(error: BaseException) -> str:
    stage = getattr(error, "failure_stage", "PREFLIGHT")
    return stage if isinstance(stage, str) and 1 <= len(stage) <= 64 else "PREFLIGHT"


def retire_preconsumption_failure(
    paths: RuntimePaths,
    action_id: str,
    error: BaseException,
    stage: str,
    *,
    ...
    counts = Counts()
    claim = _claim_owner(paths, action_id, faults)
    if claim is None:
        return Outcome(counts, True, None, "DUPLICATE_REJECTED", None, False)
    diagnostic_stage = stage if 1 <= len(stage) <= 64 else "PREFLIGHT"
    record = _terminal(
        action_id,
        ...
        evaluator_diagnostic=None,
        wrapper_diagnostic=_diagnostic(error, diagnostic_stage),
    )
    terminal_path, failed = _publish_preconsumption_terminal(paths, terminal_validator, record, faults)
...
def run_execution(
    ...
    *,
    ...
    try:
        ...
        validators["owner"](claim)
        stage = "LIFECYCLE_PREPARATION"
        authority = _authority(bindings)
        credential = _credential(action_id, authority)
        ...
        diagnostic_stage = _preflight_stage(error) if stage == "PREFLIGHT" else stage
        record = _terminal(
            action_id,
            status="PREFLIGHT_FAILED_TERMINAL",
            reason_code="READ_ONLY_PREFLIGHT_FAILED",
        ...
        terminal_path, failed = _publish_preconsumption_terminal(paths, validators.get("terminal"), record, faults)
        return Outcome(counts, False, None, record["status"], terminal_path, failed)
    ...
    try:
        ...
        record = _terminal(
            ...
            failure_stage=stage,
            evaluator_diagnostic=None,
            wrapper_diagnostic=_diagnostic(error, stage),
        )
        terminal_path, failed = _publish_preconsumption_terminal(paths, validators.get("terminal"), record, faults)
        return Outcome(counts, False, None, record["status"], terminal_path, failed)
    ...
    try:
        ...
        terminal_value = adapter_result.record["terminal"]
        record = _terminal(
            action_id,
            status=terminal_value["status"],
            reason_code=terminal_value["reason_code"],
            authority_sha256=authority["authority_sha256"],
        ...
        orphan = _terminal(
            ...
            counts=counts,
            result_sha256=result_sha256,
            failure_stage="TERMINAL_PUBLICATION",
            evaluator_diagnostic=diagnostic,
            wrapper_diagnostic=_diagnostic(OSError("terminal publication"), "TERMINAL_PUBLICATION"),
        )
        terminal_path, failed, status_value = _publish_postconsumption_terminal(
            paths, validators.get("terminal"), record, orphan, faults
        )
        ...
        orphan = _terminal(
            ...
            result_sha256=result_sha256,
            failure_stage=failure_stage,
            evaluator_diagnostic=diagnostic if type(diagnostic) is dict else None,
            wrapper_diagnostic=wrapper,
        )
        terminal_path, failed = _publish_preconsumption_terminal(paths, validators.get("terminal"), orphan, faults)
