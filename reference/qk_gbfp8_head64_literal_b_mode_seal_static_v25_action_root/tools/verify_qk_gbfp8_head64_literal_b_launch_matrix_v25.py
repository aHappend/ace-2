#!/usr/bin/env python3
"""Execute the inert positive/negative literal-``-B`` launch matrix for V25."""
...
CANDIDATE = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_literal_b_mode_seal_candidate_v25.py"
POST = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_literal_b_mode_seal_post_acceptance_v25.py"
...
def run_negative(case: dict[str, Any], entrypoint: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-v25-launch-negative-", dir=BUILD_ROOT) as directory:
...
def run_positive(output_root: Path) -> dict[str, Any]:
    candidate_report = output_root / "QK_GBFP8_HEAD64_LITERAL_B_MODE_SEAL_STATIC_V25_CANDIDATE_REPORT.json"
    negative_report = output_root / "QK_GBFP8_HEAD64_LITERAL_B_MODE_SEAL_STATIC_V25_NEGATIVE_FIXTURE_REPORT.json"
    ...
    require(candidate_value["status"] == "PASS_V25_STATIC_CANDIDATE_PENDING_FRESH_L2", "candidate status")
    require(negative_value["status"] == "PASS_V25_LITERAL_B_MODE_SEAL_AND_SYNTHETIC_MUTATION_REJECTION", "negative status")
    return {
        "action_tree_unchanged": True,
...
def main() -> int:
    ...
    try:
        ...
        raise MatrixError("launch-matrix output root must stay inside declared V25 build root") from error
    ...
    report = {
        "artifact_kind": "qk_gbfp8_head64_literal_b_mode_seal_v25_launch_matrix_report",
        ...
        "status": "PASS_V25_LITERAL_B_LAUNCH_MATRIX_PENDING_FRESH_L2",
    }
    report["report_sha256"] = sha256_bytes(compact_bytes(report))
    output = output_root / "QK_GBFP8_HEAD64_LITERAL_B_MODE_SEAL_STATIC_V25_LAUNCH_MATRIX_REPORT.json"
