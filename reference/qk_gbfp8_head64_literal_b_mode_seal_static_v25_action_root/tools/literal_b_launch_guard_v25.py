#!/usr/bin/env python3
"""Literal ``-B`` launch proof shared by the sealed V25 entry points."""
...
def prove_effective_suppression() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-v25-literal-b-probe-") as directory:
        root = Path(directory)
        source = root / "suppression_probe.py"
        source.write_bytes(b"VALUE = 25\n")
        before = _inventory(root)
        name = "ace2_v25_bytecode_suppression_probe"
