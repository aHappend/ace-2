#!/usr/bin/env python3
"""Checksum-bound V6 controller for one future isolated-root Base G16 action.
...
AUTHORITY_PACKAGE_REL = "reference/QK_GBFP8_G16_BASE_CARDINALITY_ONE_AUTHORITY_STATIC_V6_PACKAGE.json"
...
AUTHORITY_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v6-authority/base/authority.json"
CREDENTIAL_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v6-authority/base/credential.json"
LEDGER_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v6-authority/base/authority-ledger.json"
FIRST_TERMINAL_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v6-authority/base/first-terminal.json"
RESULT_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v6/base/result.json"
...
def load_historical_json(path: Path) -> Any:
    verify_plain_regular(path)
    raw = path.read_bytes()
    require(raw != b"", f"empty historical JSON {path}")
    value = json.loads(
        raw.decode("ascii"),
        object_pairs_hook=reject_duplicate_keys,
        parse_constant=lambda token: (_ for _ in ()).throw(ValueError(token)),
...
def validate_authority_package(
    ...
    *,
    ...
    controller_byte_count: int,
    review_mission_sha256: str,
    review_mission_byte_count: int,
) -> None:
    exact_keys(
        package,
        {
    ...
    require(valid_sha256(package_sha256), "authority package checksum syntax")
    require(package["authority_package_id"] == AUTHORITY_PACKAGE_ID, "authority package identity")
    require(package["schema_version"] == 6, "authority package schema version")
    projection = package["action_root_projection"]
    require(projection["action_root_id"] == ACTION_ROOT_ID, "action-root identity")
    require(projection["absolute_root"] == str(ROOT), "action-root absolute binding")
    require(projection["source_repository_root"] == str(SOURCE_REPOSITORY_ROOT), "source-root binding")
    require(projection["evaluator_argv_exactly_accepted"] is True, "accepted evaluator argv projection")
    require(projection["sealed_tensor_materialized"] is False, "sealed tensor template state")
    ...
    require(controller["sha256"] == controller_sha256, "controller checksum")
    require(controller["byte_count"] == controller_byte_count, "controller byte count")
    ...
    require(review_mission_byte_count == mission["byte_count"], "authority review mission bytes")
    require(review_mission_sha256 == mission["sha256"], "authority review mission checksum")
    ...
    require(package["v3_terminal_immutability"]["no_replay"] is True, "V3 no-replay closure")
    v5 = package["v5_failure_immutability"]
    require(v5["mission_id"] == "872248a2c6a7", "V5 failed action mission")
    require(
        v5["irreversible_action_id"]
        == "ace2:g16-base-v5:execute-once-v5ns:e326d1bf:26516d60:20260813T1140Z",
        "V5 failed action identity",
    )
    require(v5["failure_reason"] == "authority package exact keys", "V5 failure reason")
    require(v5["controller_exit_code"] == 2, "V5 controller exit")
    require(v5["credential_consumed"] is False and v5["replay_permitted"] is False, "V5 no replay")
    require(
        v5["ledger_absent"] is True
        and v5["result_absent"] is True
        and v5["first_terminal_absent"] is True,
        "V5 terminal absences",
    )
    for field in (
        "authority_sha256",
        "capture_sha256",
        ...
        require(valid_sha256(v5[field]), f"V5 {field} syntax")
    runtime = package["v6_compatibility_runtime"]
    require(runtime["package"]["sha256"] == G16_PACKAGE_SHA256, "V6 runtime package binding")
    require(runtime["evaluator"]["sha256"] == G16_EVALUATOR_SHA256, "V6 evaluator binding")
...
def validate_bound_static_artifacts(package: dict[str, Any]) -> dict[str, Any]:
    ...
    for binding in package["action_root_projection"]["accepted_dependency_projection"]:
        relative = Path(binding["projected_path"])
        require(not relative.is_absolute() and ".." not in relative.parts, "projected dependency path closure")
        require(binding["source_path"] == binding["projected_path"], "accepted relative-path preservation")
        require(binding["projected_path"] not in projected_paths, "duplicate projected dependency")
        projected_paths.add(binding["projected_path"])
        path = ROOT / relative
        verify_plain_regular(path)
        require(path.stat().st_size == binding["byte_count"], f"projected dependency bytes {path}")
        require(sha256_file(path) == binding["sha256"], f"projected dependency checksum {path}")
    for name, binding in package["schemas"].items():
        path = ROOT / binding["path"]
        verify_plain_regular(path)
        require(path.stat().st_size == binding["byte_count"], f"V2 schema byte count {name}")
        require(sha256_file(path) == binding["sha256"], f"V2 schema checksum {name}")
        schema_record = load_canonical_json(path)
        require(schema_record["schema_id"] == binding["schema_id"], f"V2 schema identity {name}")
    schema_binding = accepted["artifacts"]["result_schema"]
    require(schema_binding["sha256"] == G16_SCHEMA_SHA256, "result schema identity")
    return load_historical_json(ROOT / schema_binding["path"])


def validate_evaluator_argv_paths(package: dict[str, Any], authority_path: Path, ledger_path: Path) -> None:
    argv = package["authority_controller"]["evaluator_argv"]
    require(argv == package["base_lane"]["invocation"]["argv"], "accepted evaluator argv")
    evaluator_path = ROOT / argv[3]
    package_path = ROOT / argv[5]
    metadata_path = ROOT / argv[13]
    tensor_path = ROOT / argv[15]
    output_path = ROOT / argv[17]
    verify_plain_regular(evaluator_path)
    verify_plain_regular(package_path)
    verify_plain_regular(authority_path)
    verify_absent_plain(ledger_path)
    verify_plain_regular(metadata_path)
    verify_plain_regular(tensor_path)
    verify_absent_plain(output_path)
    lane = package["base_lane"]
    require(metadata_path.stat().st_size == lane["sealed_inputs"]["metadata"]["byte_count"], "metadata bytes")
    require(sha256_file(metadata_path) == lane["sealed_inputs"]["metadata"]["sha256"], "metadata checksum")
    require(tensor_path.stat().st_size == lane["sealed_inputs"]["tensor_bundle"]["byte_count"], "tensor bytes")
    require(package["action_root_projection"]["sealed_tensor_runtime_sha256"] == lane["sealed_inputs"]["tensor_bundle"]["sha256"], "tensor checksum binding")
...
def validate_review(review_path: Path, package_sha256: str, credential: dict[str, Any]) -> str:
    ...
    review = load_historical_json(review_path)
...
def _main() -> int:
    ...
    validate_authority_package(
        ...
        controller_byte_count=Path(__file__).stat().st_size,
        review_mission_sha256=sha256_file(AUTHORITY_REVIEW_MISSION_PATH),
        review_mission_byte_count=AUTHORITY_REVIEW_MISSION_PATH.stat().st_size,
    )
    ...
    validate_evaluator_argv_paths(package, authority_path, ledger_path)
    require(
...
def main() -> int:
    try:
        ...
        payload = {
            ...
            "schema_id": "QK_GBFP8_G16_BASE_AUTHORITY_CONTROLLER_REJECTION_V6",
        }
