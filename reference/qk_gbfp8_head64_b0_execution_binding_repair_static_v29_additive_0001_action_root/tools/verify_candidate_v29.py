...
def _reviewer_completed_replay_fixture() -> bytes:
    call_id = "1" * 32
    lines = [
        {"call_id": call_id, "run_label": "reviewer", "ts": 10.0, "type": "provider.request.started"},
        {"call_id": call_id, "prompt": f"{C.MISSION_ID} {C.STATIC_ACTION_ID}", "run_label": "reviewer", "ts": 10.1, "type": "agent.io.start", "working_dir": str(C.PROJECT_ROOT)},
        {"actor": "reviewer", "agent_layer": "reviewer", "exit_code": 0, "kind": "command_execution", "status": "completed", "ts": 10.2, "type": "engineer.progress"},
        {"call_id": call_id, "ts": 10.3, "type": "agent.io.complete"},
    ]
...
def run_regressions(package: dict[str, Any]) -> dict[str, Any]:
    ...
    _expect_rejected(
        ...
        lambda: C.derive_reviewer_provenance(_reviewer_completed_replay_fixture(), FakeInfo()),
        rejected,
    )
    ...
    baseline_acceptance = C.add_self_hash(C.acceptance_unhashed(package, dummy_report, C.compact_bytes(dummy_report), fake_provenance), "acceptance_sha256")
...
def verify_candidate(allow_acceptance: bool = False) -> dict[str, Any]:
