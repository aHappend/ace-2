#!/usr/bin/env python3
"""Literal ``-B`` launch proof shared by the sealed V28 entry points."""
...
def prove_effective_suppression() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-v28-literal-b-probe-") as directory:
        root = Path(directory)
        source = root / "suppression_probe.py"
        source.write_bytes(b"VALUE = 28\n")
        before = _inventory(root)
        name = "ace2_v28_bytecode_suppression_probe"
