#!/usr/bin/env python3
"""Bytecode-free acceptance-aware verifier for additive static-only V24."""
...
PACKAGE = ACTION_ROOT / "QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_PACKAGE.json"
...
ACCEPTANCE_SCHEMA = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_FRESH_L2_ACCEPTANCE_SCHEMA.json"
...
POST_VERIFIER = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_bytecode_free_verifier_post_acceptance_v24.py"
ACCEPTANCE = ACTION_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"
MISSION = Path("/home/argustest/.argus-skill-ace2/projects/s-c8ae985b/handoffs/v24bytecodefreeverifier/mission.json")
...
EXPECTED_ACTION_ID = "ace2:qk-gbfp8-base-v24:bytecode-free-verifier:ca93ecc0:additive-0001"
...
EXPECTED_ACTION_IDENTITY = {
    ...
    "predecessor_action_id": EXPECTED_V23_ACTION_ID,
...
EXPECTED_FRESH_L2 = {
    ...
    "acceptance_schema_file_sha256": "85ae60635a47002e4352fab0edcdd651e1accc27581a7a9eee95d126ca4ef6cd",
    "acceptance_schema_path": "reference/QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_FRESH_L2_ACCEPTANCE_SCHEMA.json",
...
EXPECTED_MISSION = {
    "path": str(MISSION),
    "sha256": "45f8a4c1d069c5cdbc89acd750b240f5b63c2f47a68e8d5a4514649638441bd3",
    "size": 3301,
}
EXPECTED_TOOLCHAIN = {
...
EXPECTED_BYTECODE_CONTRACT = {
    "candidate_entrypoint": "tools/verify_qk_gbfp8_head64_bytecode_free_verifier_candidate_v24.py",
    ...
    "post_acceptance_entrypoint": "tools/verify_qk_gbfp8_head64_bytecode_free_verifier_post_acceptance_v24.py",
...
EXPECTED_FORBIDDEN_PATHS = [
    ...
    str(PROJECT_ROOT / "runtime/qk_gbfp8_head64_bytecode_free_verifier_v24_ca93ecc0"),
...
EXPECTED_STATIC_POLICY = {
    ...
    "allowed_relative_files": [
        "QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_PACKAGE.json",
        ...
        "reference/QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_FRESH_L2_ACCEPTANCE_SCHEMA.json",
        "review/FRESH_L2_STATIC_ACCEPTANCE.json",
        "tools/validate_qk_gbfp8_head64_source_oracle_b0_v23.py",
        "tools/verify_qk_gbfp8_head64_bytecode_free_verifier_candidate_v24.py",
        "tools/verify_qk_gbfp8_head64_bytecode_free_verifier_post_acceptance_v24.py",
    ],
...
def audit_hook(event: str, args: tuple[Any, ...]) -> None:
    if event == "open" and args:
        ...
        if _inside(path, ACTION_ROOT) and _open_is_mutating(args):
            raise VerificationError(f"V24 action-tree write prohibited: {path}")
    ...
    if event in {"os.remove", "os.rmdir", "os.mkdir", "os.rename", "os.replace", "os.chmod", "os.link", "os.symlink"}:
        for value in args[:2]:
            ...
            if _inside(path, ACTION_ROOT):
                raise VerificationError(f"V24 action-tree mutation prohibited: {event}:{path}")
    if event == "subprocess.Popen":
        AUDIT["official_target_process_starts"] += 1
        raise VerificationError("process start prohibited in inert V24 verifier")
...
def verify_launch_contract() -> dict[str, Any]:
    require(sys.flags.dont_write_bytecode == 1, "V24 requires interpreter -B")
    require(os.environ.get("PYTHONDONTWRITEBYTECODE") == "1", "V24 requires PYTHONDONTWRITEBYTECODE=1")
    require(sys.dont_write_bytecode is True, "V24 sys.dont_write_bytecode must be true")
    require(sys.pycache_prefix is None, "V24 rejects PYTHONPYCACHEPREFIX")
    with tempfile.TemporaryDirectory(prefix="ace2-v24-bytecode-probe-") as directory:
        root = Path(directory)
        source = root / "suppression_probe.py"
        source.write_bytes(b"VALUE = 24\n")
        before = inventory_records(root)
        module_name = "ace2_v24_bytecode_suppression_probe"
        try:
            load_module(source, module_name)
        finally:
            sys.modules.pop(module_name, None)
        after = inventory_records(root)
        require(after == before, "bytecode suppression ineffective during import probe")
    return {
        "environment": {"PYTHONDONTWRITEBYTECODE": os.environ["PYTHONDONTWRITEBYTECODE"]},
        "import_probe_exact_tree_match": True,
        "interpreter_flags": ["-B"],
...
def validate_static_contract(package: dict[str, Any]) -> None:
    require(package["artifact_kind"] == "qk_gbfp8_head64_bytecode_free_verifier_static_v24_package", "package kind")
    require(package["authoritative_stage"] == "Base", "authoritative stage")
    require(package["root_id"] == "qk_gbfp8_head64_bytecode_free_verifier_static_v24_action_root", "root id")
...
def verify_inventory(package: dict[str, Any], allow_acceptance: bool) -> dict[str, Any]:
    ...
    for path in sorted(ACTION_ROOT.rglob("*")):
        info = os.lstat(path)
        require(not stat.S_ISLNK(info.st_mode), f"V24 symlink: {path}")
        relative = path.relative_to(ACTION_ROOT).as_posix()
        if stat.S_ISDIR(info.st_mode):
            observed_directories.add(relative)
            continue
        require(stat.S_ISREG(info.st_mode), f"V24 non-regular file: {relative}")
    ...
    require(observed == expected, f"V24 inventory expected={sorted(expected)} observed={sorted(observed)}")
    require(observed_directories == expected_directories, f"V24 directories expected={sorted(expected_directories)} observed={sorted(observed_directories)}")
    records = inventory_records(ACTION_ROOT)
    assert_no_forbidden_inventory(records, package["bytecode_free_verifier_contract"], "V24 inventory")
    normalized = [record for record in records if record["path"] != acceptance_relative]
    return {
...
def verify_v22_and_predecessors() -> dict[str, Any]:
    ...
    v22 = load_module(V22_CANDIDATE_VERIFIER, "v24_bound_v22_candidate_verifier")
...
def verify_package_mutations(package: dict[str, Any]) -> dict[str, Any]:
    ...
    for section in sections:
        for relative in _scalar_paths(package[section]):
            ...
            if section == "static_file_policy" and original == EXPECTED_BYTECODE_CONTRACT["normal_import_negative_fixture"]:
                continue
    ...
    require(len(rejected) == 187, f"carried V23 package mutation count: {len(rejected)}")
...
def verify_v24_extension_mutations(package: dict[str, Any]) -> dict[str, Any]:
    rejected = []
    for section in ("v23_preservation", "bytecode_free_verifier_contract"):
        for relative in _scalar_paths(package[section]):
            changed = deepcopy(package)
            path = (section,) + relative
            original = changed
            ...
            try:
                ...
                raise VerificationError(f"fail-closed V24 extension mutation accepted: {path}")
    changed = deepcopy(package)
    ...
    try:
        ...
        raise VerificationError("fail-closed V24 fixture inventory mutation accepted")
...
def verify_v23_style_normal_import_fixture() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-v24-normal-import-negative-") as directory:
        root = Path(directory)
...
def verify_no_live_effects(package: dict[str, Any]) -> dict[str, Any]:
    for value in package["forbidden_live_paths"]:
        require(not os.path.lexists(value), f"forbidden V24 live path exists: {value}")
...
def build_negative_report(package: dict[str, Any]) -> dict[str, Any]:
    ...
    report = {
        "artifact_kind": "qk_gbfp8_head64_bytecode_free_verifier_v24_synthetic_negative_fixture_report",
        ...
        "v24_extension_mutations": verify_v24_extension_mutations(package),
        "status": "PASS_V24_BYTECODE_AND_SYNTHETIC_MUTATION_REJECTION",
    }
...
def verify(allow_acceptance: bool = False) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    launch = verify_launch_contract()
    action_before = inventory_records(ACTION_ROOT)
    package, package_raw = verify_manifest()
    assert_no_forbidden_inventory(action_before, package["bytecode_free_verifier_contract"], "V24 action tree before verifier")
    negative = build_negative_report(package)
    inventory = verify_inventory(package, allow_acceptance)
    report = {
        "artifact_kind": "qk_gbfp8_head64_bytecode_free_verifier_static_v24_candidate_verifier_report",
        ...
        "status": "PASS_V24_STATIC_CANDIDATE_PENDING_FRESH_L2",
        "v23_preservation": verify_v23_preservation(),
        "v24_execution_authority_granted": False,
    }
    action_after = inventory_records(ACTION_ROOT)
    require(action_after == action_before, "V24 candidate verifier mutated exact action tree")
    ...
    report["inventory_guard"] = {
        ...
        "candidate_action_tree_before_sha256": sha256_bytes(compact_bytes(normalized)),
        "candidate_action_tree_exact_match": True,
        "candidate_action_tree_after_sha256": sha256_bytes(compact_bytes(normalized)),
    }
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=BUILD_ROOT / "QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_CANDIDATE_REPORT.json")
    parser.add_argument("--negative-output", type=Path, default=BUILD_ROOT / "QK_GBFP8_HEAD64_BYTECODE_FREE_VERIFIER_STATIC_V24_NEGATIVE_FIXTURE_REPORT.json")
    ...
    for path in (args.output, args.negative_output):
        try:
            ...
            raise VerificationError(f"V24 report output must stay inside declared build root: {path}") from error
