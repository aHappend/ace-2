...
ACTION_ROOT = PROJECT_ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v16_shellfree_action_root"
RUNTIME_ROOT = PROJECT_ROOT / "runtime/qk_gbfp8_head64_granularity_sweep_execution_v16_7f7e672a"
...
def verify_fixture_source() -> dict[str, Any]:
    ...
    return {"imports": sorted(imports), "sha256": sha256_file(FIXTURE)}
...
def verify_fixture_report() -> dict[str, Any]:
    ...
    require(type(cases) is list and len(cases) >= 11, "fixture case count")
    ...
    return {"case_count": len(cases), "raw_sha256": sha256_bytes(raw), "stages": sorted(observed_stages)}
