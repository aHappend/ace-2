#!/usr/bin/env python3
"""Final inert verifier for one independently accepted, mode-sealed V26."""
...
def _load_bound_launch_guard() -> Any:
    ...
    name = "ace2_v27_post_bound_literal_b_launch_guard"
...
PROVENANCE_AUDIT = BUILD_ROOT / "fresh-l2-provenance-audit-0001/QK_GBFP8_HEAD64_ROLE_ENFORCED_ACCEPTANCE_STATIC_V27_PROVENANCE_AUDIT.json"
...
def inventory_records(root: Path) -> list[dict[str, Any]]:
    ...
    if not stat.S_ISDIR(info.st_mode) or stat.S_ISLNK(info.st_mode):
        raise RuntimeError("V26 action root absent, linked, or non-directory")
    records: list[dict[str, Any]] = [{"kind": "directory", "mode": f"{stat.S_IMODE(info.st_mode):04o}", "path": "."}]
    for path in sorted(root.rglob("*")):
        info = os.lstat(path)
        relative = path.relative_to(root).as_posix()
        if stat.S_ISLNK(info.st_mode):
            raise RuntimeError(f"V26 action-tree symlink: {relative}")
        ...
        if stat.S_ISDIR(info.st_mode):
            ...
            raise RuntimeError(f"V26 unsupported action-tree entry: {relative}")
    return records


PRE_IMPORT_TREE = inventory_records(ACTION_ROOT)

import verify_qk_gbfp8_head64_role_enforced_acceptance_candidate_v27 as candidate
...
def verify_acceptance(package: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate.require(candidate_report["status"] == "PASS_V27_STATIC_CANDIDATE_PENDING_FRESH_L2", "accepted candidate status")
    ...
    candidate.require(negative_report["status"] == "PASS_V27_LITERAL_B_MODE_SEAL_AND_SYNTHETIC_MUTATION_REJECTION", "accepted negative status")
    ...
    candidate.require(matrix_report["status"] == "PASS_V27_LITERAL_B_LAUNCH_MATRIX_PENDING_FRESH_L2", "accepted launch matrix status")
    ...
    expected_dynamic = {
        ...
        "artifact_kind": "qk_gbfp8_head64_role_enforced_acceptance_static_v27_fresh_l2_acceptance",
        ...
        "v25_action_tree_sha256": package["v25_preservation"]["v25_action_root"]["tree_sha256"],
        "v25_package_file_sha256": package["v25_preservation"]["package"]["sha256"],
        "v26_acceptance_file_sha256": package["v26_preservation"]["acceptance"]["sha256"],
        "v26_action_tree_sha256": package["v26_preservation"]["v26_action_root"]["tree_with_root_sha256"],
        "v26_package_file_sha256": package["v26_preservation"]["package"]["sha256"],
        "v26_positive_post_report_file_sha256": package["v26_preservation"]["positive_post_report"]["sha256"],
    }
    ...
    audit, audit_raw = candidate.canonical(PROVENANCE_AUDIT)
    candidate.verify_self_hash(audit, "report_sha256")
    candidate.require(audit["acceptance_file_sha256"] == sha256_bytes(raw), "provenance audit acceptance binding")
    candidate.require(audit["acceptance_sha256"] == acceptance["acceptance_sha256"], "provenance audit acceptance self-hash binding")
    candidate.require(audit["creator_provenance"] == acceptance["creator_provenance"], "provenance audit identity object binding")
    candidate.require(audit["provenance_identity_sha256"] == acceptance["provenance_identity_sha256"] == acceptance["creator_provenance"]["provenance_identity_sha256"], "provenance audit identity hash binding")
    candidate.require(audit["status"] == "PASS_V27_AUTHORITATIVE_REVIEWER_PROVENANCE_AUDIT", "provenance audit status")
    return {
...
def verify() -> dict[str, Any]:
    ...
    candidate.verify_v25_preservation()
    candidate.verify_v26_preservation()
    candidate.verify_role_enforcement()
    ...
    candidate.require(post_tree == PRE_IMPORT_TREE, "V26 post-acceptance verifier mutated exact final tree")
    tree_sha256 = sha256_bytes(compact_bytes(post_tree))
    report = {
        "acceptance": acceptance,
        "artifact_kind": "qk_gbfp8_head64_role_enforced_acceptance_static_v27_post_acceptance_inert_report",
        "claim_boundary": candidate.EXPECTED_CLAIM,
        ...
        "status": "PASS_V27_STATIC_POST_ACCEPTANCE_ROLE_ENFORCED_LITERAL_B_MODE_SEALED",
...
def main() -> int:
    ...
    try:
        ...
        raise candidate.VerificationError("V26 post report must stay inside declared build root") from error
