...
SOURCE_FILES = [
    ...
    "tools/reviewer_preflight_v29.py",
...
def refresh_schema_bindings() -> None:
    manager_path = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_B0_V29_MANAGER_ADMISSION_CAPSULE_SCHEMA.json"
    operator_path = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_B0_V29_OPERATOR_AUTHORITY_CAPSULE_SCHEMA.json"
    for path in (manager_path, operator_path):
        value = json.loads(path.read_text(encoding="ascii"))
        claim = value["$defs"]["claim"]["properties"]
        claim["static_action_id"] = {"const": C.STATIC_ACTION_ID}
        claim["execution_action_id"] = {"const": C.EXECUTION_ACTION_ID}
        path.write_bytes(C.compact_bytes(value))

    acceptance_path = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_B0_V29_FRESH_L2_ACCEPTANCE_SCHEMA.json"
    acceptance = json.loads(acceptance_path.read_text(encoding="ascii"))
    acceptance["properties"]["action_id"] = {"const": C.STATIC_ACTION_ID}
    provenance = acceptance["$defs"]["reviewer_provenance"]
    provenance["properties"]["action_id"] = {"const": C.STATIC_ACTION_ID}
    hash_fields = [
        "candidate_action_tree_sha256",
        "candidate_report_file_sha256",
        "candidate_report_path_sha256",
        "candidate_report_sha256",
        "execution_binding_sha256",
        "package_content_sha256",
        "package_file_sha256",
        "reviewer_preflight_command_sha256",
        "reviewer_preflight_commitment_sha256",
        "reviewer_preflight_cwd_sha256",
        "reviewer_preflight_script_sha256",
    ]
    for field in hash_fields:
        provenance["properties"][field] = {"pattern": "^[0-9a-f]{64}$", "type": "string"}
        if field not in provenance["required"]:
            provenance["required"].append(field)
    provenance["properties"]["claim_boundary"] = {"const": C.CLAIM_BOUNDARY}
    if "claim_boundary" not in provenance["required"]:
        provenance["required"].append("claim_boundary")
    acceptance_path.write_bytes(C.compact_bytes(acceptance))
...
def build_package() -> dict[str, Any]:
    ...
    package: dict[str, Any] = {
        "action_identity": {
            ...
            "root_id": "qk_gbfp8_head64_b0_execution_binding_repair_static_v29_additive_0005_action_root",
        ...
        "authority_contract": {
            ...
            "event_source": {
                "exact_live_frontier_equality_required": True,
                "id": "ARGUS_PROJECT_EVENTS_JSONL",
                "path": str(C.EVENT_LOG),
                "post_consumption_recheck_before_envelope_and_protected_start_required": True,
                "replacement_symlink_truncation_or_prefix_drift_rejected": True,
                "single_stable_read_shared_by_both_capsule_sources": True,
            },
            ...
            "required_bindings": [
                ...
                "exact live line and byte frontier equality",
                "post-consumption exact event identity/frontier recheck before envelope publication and protected start",
        ...
        "fresh_l2_review": {
            ...
            "acceptance_creator_command": C.reviewer_acceptance_creator_command(),
            "acceptance_creator_working_directory": str(ACTION_ROOT),
            "post_generated_identity_binding": "AUTHORITATIVE_REVIEWER_PREFLIGHT_COMMAND_EVENT",
            "reviewer_preflight_command": C.reviewer_preflight_command(),
            "reviewer_preflight_command_sha256": C.sha256_bytes((C.reviewer_preflight_command() + "\n").encode("utf-8")),
            "reviewer_preflight_working_directory": str(ACTION_ROOT),
            "scheduler_start_binding": "IMMUTABLE_MISSION_ONLY",
        ...
        "predecessor_preservation": {
            ...
            "rejected_v29_additive_0001": {
                "accepted_lineage": False,
                "action_id": "ace2:qk-gbfp8-base-v29:b0-execution-binding-repair:c4a9f2d1:additive-0001",
                "package_content_sha256": C.V29_REJECTED_CONTENT_SHA256,
                "package_file_sha256": C.V29_REJECTED_PACKAGE_SHA256,
                "preacceptance_tree_sha256": C.V29_REJECTED_TREE_SHA256,
                "root": str(C.V29_REJECTED_ROOT),
                "status": "PERMANENTLY_REJECTED_PROVENANCE_AND_PATHNAME_RACE_EVIDENCE_ONLY",
            },
            "rejected_v29_additive_0002": {
                "accepted_lineage": False,
                "action_id": "ace2:qk-gbfp8-base-v29:b0-execution-binding-repair:c4a9f2d1:additive-0002",
                "execution_binding_sha256": C.V29_REJECTED_0002_EXECUTION_BINDING_SHA256,
                "package_content_sha256": C.V29_REJECTED_0002_CONTENT_SHA256,
                "package_file_sha256": C.V29_REJECTED_0002_PACKAGE_SHA256,
                "preacceptance_tree_sha256": C.V29_REJECTED_0002_TREE_SHA256,
                "root": str(C.V29_REJECTED_0002_ROOT),
                "status": "PERMANENTLY_REJECTED_FRONTIER_AND_DUPLICATE_START_FAILURE_EVIDENCE_ONLY",
            },
            "rejected_v29_additive_0003": {
                "accepted_lineage": False,
                "action_id": "ace2:qk-gbfp8-base-v29:b0-execution-binding-repair:c4a9f2d1:additive-0003",
                "execution_binding_sha256": C.V29_REJECTED_0003_EXECUTION_BINDING_SHA256,
                "package_content_sha256": C.V29_REJECTED_0003_CONTENT_SHA256,
                "package_file_sha256": C.V29_REJECTED_0003_PACKAGE_SHA256,
                "preacceptance_tree_sha256": C.V29_REJECTED_0003_TREE_SHA256,
                "root": str(C.V29_REJECTED_0003_ROOT),
                "status": "PERMANENTLY_REJECTED_POST_VALIDATION_FRONTIER_AND_SAME_INODE_REWRITE_EVIDENCE_ONLY",
            },
        },
        "regression_contract": {
            ...
            "required_cases": [
                ...
                "unrelated 51-byte append after bound authority frontier",
                "post-consumption event append/truncation/replacement before envelope publication or protected start",
                "stale/completed/replayed authority",
                "capsule substitution between lstat/open and before final move, same-inode rewrite, and metadata drift",
                "event-log replacement after open before read completion",
                "scheduler-feasible Reviewer mission start plus authoritative preflight command binding",
                "runtime preexistence and real atomic duplicate starts terminally blocking every claimant",
                "SOURCE_ORACLE_MATCH and each of 574 single-row mismatches",
                "safe aggregate-only output and zero official effects",
            ],
        },
        "runtime_ordering": {
            "claim_liveness_rechecked_before_every_mutation_and_protected_start": True,
            "consumed_capsule_snapshot_rechecked_before_envelope_and_protected_start": True,
            "duplicate_start_terminal_materialized_inside_claimed_runtime_and_sibling": True,
            "event_log_identity_and_exact_frontier_rechecked_before_envelope_and_protected_start": True,
            ...
            "ordered_steps": [
                ...
                "initialize owner-bound claim and reject any runtime or sibling terminal",
                "verify and atomically consume external Manager and operator capsules once with claim-liveness guards",
                "recheck moved capsule bytes/metadata and one shared exact event-log identity/frontier before writing the frozen envelope",
                "write and freeze internal authority envelope with claim-liveness guards",
                "recheck moved capsule bytes/metadata, event identity/frontier, and claim liveness immediately before crossing protected payload/evaluator/target boundary",
            ],
            "runtime_namespace_must_not_preexist": True,
        },
        "schema_version": 1,
        "static_review_output_paths": {
            "candidate_report": str(C.CANDIDATE_REPORT),
            "post_report": str(C.PROJECT_ROOT / "build/v29-b0-execution-binding-repair-static-0005/fresh-l2-post-0005/V29_POST_ACCEPTANCE_REPORT.json"),
        },
