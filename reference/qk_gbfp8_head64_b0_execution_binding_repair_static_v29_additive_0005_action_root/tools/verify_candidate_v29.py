...
def _run_real_pathname_races(rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0005-races-", dir="/tmp") as temporary:
        root = Path(temporary)
...
def _run_post_consumption_event_races(package: dict[str, Any], rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0005-post-consumption-", dir="/tmp") as temporary:
        root = Path(temporary)
...
def _run_real_atomic_duplicate_start(rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0005-duplicate-start-", dir="/tmp") as temporary:
        root = Path(temporary)
        runtime = root / "runtime"
        failure_seal = root / "runtime.failed-start"
        original_runtime = W.C.RUNTIME_NAMESPACE
