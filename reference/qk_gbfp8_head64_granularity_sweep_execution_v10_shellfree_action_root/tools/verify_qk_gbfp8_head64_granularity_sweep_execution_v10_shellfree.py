#!/usr/bin/env python3
"""Inert independent verifier for the Base V10 canonical-byte successor.
...
ACTION_ROOT = PROJECT_ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree_action_root"
PACKAGE_PATH = ACTION_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_PACKAGE.json"
LAUNCHER_PATH = ACTION_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v10.py"
TRANSPORT_PATH = ACTION_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v10.py"
...
EXPECTED_PACKAGE_KIND = "qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree_package"
...
def verify_manifest_contract(package: dict[str, Any], *, files: bool) -> None:
    ...
    if files:
        canonical = binding_map(package, "canonical_bindings")
        local = binding_map(package, "local_artifact_bindings")
        require(canonical["package"]["path"] == str(CANONICAL_PACKAGE), "canonical package binding path")
        require(canonical["result_schema"]["path"] == str(CANONICAL_RESULT_SCHEMA), "canonical schema binding path")
        require(canonical["package_predecessor_raw"]["path"] == str(V8_PACKAGE_RAW), "predecessor package binding")
        require(canonical["result_schema_predecessor_raw"]["path"] == str(V8_RESULT_SCHEMA_RAW), "predecessor schema binding")
        require(canonical["v9_first_terminal"]["path"] == str(V9_TERMINAL), "V9 terminal binding")
        for record in [*canonical.values(), *local.values()]:
        ...
        require(local["launcher"]["path"] == str(LAUNCHER_PATH), "launcher local path")
        require(local["transport"]["path"] == str(TRANSPORT_PATH), "transport local path")
...
def verify_acceptance_if_present(package_raw: bytes) -> dict[str, Any] | None:
    ...
    require(acceptance.get("artifact_kind") == "qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree_fresh_l2_static_acceptance", "acceptance kind")
...
def main() -> int:
    ...
    result = {
        ...
        "status": "PASS_V10_CANONICAL_PREFLIGHT_SUCCESSOR_INDEPENDENT_STATIC_VERIFICATION",
