...
def _move_consumed_inode(path: Any, destination: Path, opened: os.stat_result, role: str) -> None:
...
def _consume_capsule(path: Path, destination: Path, role: str, package: dict[str, Any], now_ts: float, runtime: Path, claim_token: str) -> tuple[dict[str, Any], bytes, dict[str, Any]]:
    _assert_claim_live(runtime, claim_token)
    ...
    _assert_claim_live(runtime, claim_token)
    _move_consumed_inode(path, destination, before, role)
    return value, raw, proof


def _write_frozen_envelope(runtime: Path, claim_token: str, package: dict[str, Any], acceptance: dict[str, Any], manager: tuple[dict[str, Any], bytes, dict[str, Any]], operator: tuple[dict[str, Any], bytes, dict[str, Any]]) -> Path:
    _assert_claim_live(runtime, claim_token)
...
def _protected_boundary(runtime: Path, claim_token: str, envelope: Path, package: dict[str, Any]) -> None:
    _assert_claim_live(runtime, claim_token)
...
def main() -> int:
    ...
    try:
        ...
        _protected_boundary(runtime, claim_token, envelope, package)
        return 0
