...
import qk_gbfp8_head64_granularity_sweep_execute_once_wrapper_v17 as launcher
...
import qk_gbfp8_head64_granularity_sweep_transport_wrapper_v17 as transport
...
def validators() -> dict[str, Any]:
    ...
    names = {
        "owner": "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V17_EXACTLY_ONCE_OWNER_CLAIM_SCHEMA.json",
...
def invoker(scenario: str) -> Callable[..., Any]:
    def invoke(payload: bytes, authority_sha256: str, ledger_sha256: str, publish: Any) -> Any:
        ...
        spec = adapter.InvocationSpec(
            argv=("synthetic-v17-adapter", scenario),
            cwd="/synthetic",
            environment=(("LANG", "C"),),
...
def run_case(name: str, operation: Callable[[RuntimePaths], dict[str, Any]]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="v17-wrapper-repair-fixture-") as temporary:
...
def transport_failure(paths: RuntimePaths, scenario: str) -> dict[str, Any]:
    ...
    launcher_hash_provider: Callable[[Path], str] = transport.sha256_file
