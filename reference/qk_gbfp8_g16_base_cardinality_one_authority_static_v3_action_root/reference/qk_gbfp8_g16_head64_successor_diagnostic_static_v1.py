#!/usr/bin/env python3
"""Deterministic G16 Q/K diagnostic reference for a later authorized run.
...
OUTPUT_ROOT_REL = "build/qk-gbfp8-g16-head64-successor-diagnostic-v1"
...
def read_exact(handle: Any, size: int) -> bytes:
    ...
    require(len(payload) == size, "truncated tensor bundle", "INPUT_BINDING_REJECTED")
    return payload
...
def read_tensor_bundle(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("rb") as handle:
        ...
        count = int.from_bytes(read_exact(handle, 4), "little")
        require(1 <= count <= 256, "tensor record count")
        for _ in range(count):
            name_size = int.from_bytes(read_exact(handle, 4), "little")
            ...
            dtype_size = int.from_bytes(read_exact(handle, 4), "little")
            require(1 <= dtype_size <= 64, "tensor dtype size")
            dtype = read_exact(handle, dtype_size).decode("ascii")
            rank = int.from_bytes(read_exact(handle, 4), "little")
            require(1 <= rank <= 8, "tensor rank")
            shape = tuple(int.from_bytes(read_exact(handle, 8), "little") for _ in range(rank))
            require(all(1 <= dimension <= (1 << 20) for dimension in shape), "tensor shape")
            payload_size = int.from_bytes(read_exact(handle, 8), "little")
        ...
        require(handle.read(1) == b"", "trailing tensor bundle bytes")
    return records
