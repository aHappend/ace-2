...
FUTURE_RUNTIME_NAMESPACE = Path('/home/argustest/ace-2/runtime/qk_gbfp8_head64_b0_source_oracle_identity_control_v28_38571d38_additive_0007_once')
ACCEPTANCE_CREATOR_RELATIVE = 'build/v28-b0-source-oracle-identity-control-static-0012/create_fresh_l2_static_acceptance_v28.py'
EXPECTED_REVIEW_ROOT = Path('/home/argustest/ace-2/build/v28-b0-source-oracle-identity-control-static-0012/fresh-l2-sealed-0001')
EXPECTED_ACTION_ROOT = Path('/home/argustest/ace-2/reference/qk_gbfp8_head64_b0_source_oracle_identity_control_static_v28_additive_0007_action_root')
...
def validate_package(package: dict[str, Any]) -> None:
    ...
    require(package["predecessor_preservation"]["failed_additive_predecessor"]["status"] == 'IMMUTABLE_FAILED_SEALED_V28_ADDITIVE_0006', "failed additive predecessor semantics")
...
def authority_baseline() -> dict[str, Any]:
    ...
    value = {
        ...
        "identity_bundle_sha256": 'fc5ebfeacbaf6a7b81040d56f4ba0f58ba75227aab12d23d72f5787dfe3db26f',
...
def verify_reviewer_creator_binding(package: dict[str, Any]) -> dict[str, Any]:
    relative = package["role_enforcement"]["acceptance_creator_path"]
    require(relative == ACCEPTANCE_CREATOR_RELATIVE, "acceptance creator package path")
    creator = PROJECT_ROOT / relative
    require(creator.is_file(), "acceptance creator absent")
    source = creator.read_text(encoding="ascii")
    expected_lines = {
        f'BUILD_ROOT = PROJECT_ROOT / "{EXPECTED_REVIEW_ROOT.parent.relative_to(PROJECT_ROOT)}"',
        f'ACTION_ROOT = PROJECT_ROOT / "{EXPECTED_ACTION_ROOT.relative_to(PROJECT_ROOT)}"',
        'REPORT_ROOT = BUILD_ROOT / "fresh-l2-sealed-0001"',
    }
    lines = set(source.splitlines())
    require(expected_lines <= lines, "acceptance creator constants")
    require(source.count("BUILD_ROOT = PROJECT_ROOT /") == 1, "acceptance creator build root ambiguity")
    require(source.count("REPORT_ROOT = BUILD_ROOT /") == 1, "acceptance creator report root ambiguity")
    paths = package["static_review_output_paths"]
    require(Path(paths["review_root"]) == EXPECTED_REVIEW_ROOT, "package review root")
    require(Path(paths["candidate_report"]) == EXPECTED_REVIEW_ROOT / "QK_GBFP8_HEAD64_SOURCE_ORACLE_IDENTITY_CONTROL_STATIC_V28_CANDIDATE_REPORT.json", "package candidate report")
    require(Path(paths["negative_report"]) == EXPECTED_REVIEW_ROOT / "QK_GBFP8_HEAD64_SOURCE_ORACLE_IDENTITY_CONTROL_STATIC_V28_NEGATIVE_FIXTURE_REPORT.json", "package negative report")
    require(Path(paths["launch_matrix_report"]) == EXPECTED_REVIEW_ROOT / "QK_GBFP8_HEAD64_SOURCE_ORACLE_IDENTITY_CONTROL_STATIC_V28_LAUNCH_MATRIX_REPORT.json", "package launch report")
    return {
        "acceptance_creator_file_sha256": sha256_file(creator),
        "acceptance_creator_relative_path": relative,
        "package_bound_review_root": str(EXPECTED_REVIEW_ROOT),
        "report_root_exact_match": True,
    }
...
def verify_candidate(action_root: Path, allow_acceptance: bool, launch_proof: dict[str, Any] | None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    report = {
        ...
        "reviewer_creator_binding": verify_reviewer_creator_binding(package),
        "row_class_count": 4,
