#!/usr/bin/env python3
"""Inert independent verifier for the Base V15 writable-runtime successor.
...
ACTION_ROOT = PROJECT_ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v15_shellfree_action_root"
PACKAGE_PATH = ACTION_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V15_SHELLFREE_PACKAGE.json"
LAUNCHER_PATH = ACTION_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v15.py"
TRANSPORT_PATH = ACTION_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v15.py"
ACCEPTANCE_PATH = ACTION_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"
ACCEPTANCE_SCHEMA_PATH = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V15_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json"
...
EXPECTED_PACKAGE_KIND = "qk_gbfp8_head64_granularity_sweep_execution_v15_shellfree_package"
...
def verify_manifest_contract(package: dict[str, Any], *, files: bool) -> None:
    ...
    require(package.get("prior_static_candidate_state") == {
    ...
    }, "prior V14 static candidate state")
    ...
    require(canonical_bindings["v14_failed_package"]["path"] == str(V14_FAILED_PACKAGE), "V14 failed package binding")
    require(canonical_bindings["v14_failure_record"]["path"] == str(V14_FAILURE_RECORD), "V14 failure record binding")
...
def manifest_adversarial_checks(package: dict[str, Any]) -> int:
    ...
    add(lambda value: value["prior_static_candidate_state"].__setitem__("acceptance_materialized", True))
    add(lambda value: value["prior_static_candidate_state"].__setitem__("manifest_raw_sha256", "0" * 64))
...
def prove_create_once(parent: Path, label: str) -> dict[str, Any]:
    probe = parent / f".v15-independent-{label}-create-once-probe"
...
def verify_v14_static_failure() -> dict[str, Any]:
    require(mode_octal(V14_FAILED_ROOT) == "0555", "V14 failed root mode")
    require(not os.path.lexists(V14_FAILED_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"), "V14 acceptance appeared")
    failure = json.loads(V14_FAILURE_RECORD.read_text(encoding="utf-8"))
    require(failure["classification"] == "PRE_REVIEW_STATIC_VERIFIER_GAP", "V14 failure classification")
    require(failure["manifest_raw_sha256"] == sha256_file(V14_FAILED_PACKAGE), "V14 failure package binding")
    require(failure["authority_materialized"] is False and failure["target_process_starts"] == 0, "V14 failure authority/start boundary")
    return {
        "action_id": failure["action_id"],
        "failure_record_sha256": sha256_file(V14_FAILURE_RECORD),
        "manifest_raw_sha256": failure["manifest_raw_sha256"],
    }
...
def verify_acceptance_if_present(package_raw: bytes) -> dict[str, Any] | None:
    ...
    require(acceptance.get("artifact_kind") == "qk_gbfp8_head64_granularity_sweep_execution_v15_shellfree_fresh_l2_static_acceptance", "acceptance kind")
...
def main() -> int:
    ...
    v14_static_failure = verify_v14_static_failure()
    ...
    result = {
        ...
        "status": "PASS_V15_WRITABLE_RUNTIME_SUCCESSOR_INDEPENDENT_STATIC_VERIFICATION",
        "target_process_starts": 0,
        "v13_retirement": v13_retirement,
        "v14_static_failure": v14_static_failure,
    }
