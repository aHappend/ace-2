...
def _load_bound_launch_guard() -> Any:
    ...
    name = "ace2_v26_bound_literal_b_launch_guard"
...
CANONICAL_ACTION_ROOT = PROJECT_ROOT / "reference/qk_gbfp8_head64_launch_harness_repair_static_v26_action_root"
BUILD_ROOT = PROJECT_ROOT / "build/v26-launch-harness-repair-static-0003"
PACKAGE = ACTION_ROOT / "QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_PACKAGE.json"
...
ACCEPTANCE_SCHEMA = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_FRESH_L2_ACCEPTANCE_SCHEMA.json"
...
POST_VERIFIER = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_launch_harness_repair_post_acceptance_v26.py"
LAUNCH_MATRIX = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_literal_b_launch_matrix_v26.py"
ACCEPTANCE = ACTION_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"
MISSION = Path("/home/argustest/.argus-skill-ace2/projects/s-c8ae985b/handoffs/v26launchharnessrepair/mission.json")
...
EXPECTED_ACTION_ID = "ace2:qk-gbfp8-base-v26:launch-harness-repair:1e2b5550:additive-0003"
...
EXPECTED_ACTION_IDENTITY = {
    ...
    "predecessor_action_id": EXPECTED_V25_ACTION_ID,
...
EXPECTED_FRESH_L2 = {
    ...
    "acceptance_schema_file_sha256": "d6d636ca0deb3e04279573be793a49a70b5a73a7198c76b167cafeaf05afa733",
    "acceptance_schema_path": "reference/QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_FRESH_L2_ACCEPTANCE_SCHEMA.json",
...
EXPECTED_MISSION = {
    "path": str(MISSION),
    "sha256": "8bc6b33e552beb54fdb1a8784ce1772c845043ceb957bc6c75f337afebded4b2",
    "size": 3695,
}
EXPECTED_TOOLCHAIN = {
...
EXPECTED_BYTECODE_CONTRACT = {
    "candidate_entrypoint": "tools/verify_qk_gbfp8_head64_launch_harness_repair_candidate_v26.py",
    ...
    "post_acceptance_entrypoint": "tools/verify_qk_gbfp8_head64_launch_harness_repair_post_acceptance_v26.py",
...
EXPECTED_LITERAL_B_CONTRACT = {
    ...
    "launch_guard": "tools/literal_b_launch_guard_v26.py",
    "launch_guard_file_sha256": "ee624cdad6eb91b0e7a1efd9a99d10be7b2d0f773d38dc1f2c87233d4b43504f",
    "launch_matrix": "tools/verify_qk_gbfp8_head64_literal_b_launch_matrix_v26.py",
...
EXPECTED_FORBIDDEN_PATHS = [
    ...
    str(PROJECT_ROOT / "runtime/qk_gbfp8_head64_launch_harness_repair_v26_1e2b5550"),
...
EXPECTED_STATIC_POLICY = {
    ...
    "allowed_relative_files": [
        "QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_PACKAGE.json",
        ...
        "reference/QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_FRESH_L2_ACCEPTANCE_SCHEMA.json",
        "review/FRESH_L2_STATIC_ACCEPTANCE.json",
        "tools/validate_qk_gbfp8_head64_source_oracle_b0_v23.py",
        "tools/literal_b_launch_guard_v26.py",
        "tools/verify_qk_gbfp8_head64_literal_b_launch_matrix_v26.py",
        "tools/verify_qk_gbfp8_head64_launch_harness_repair_candidate_v26.py",
        "tools/verify_qk_gbfp8_head64_launch_harness_repair_post_acceptance_v26.py",
    ],
...
def validate_static_contract(package: dict[str, Any]) -> None:
    require(package["artifact_kind"] == "qk_gbfp8_head64_launch_harness_repair_static_v26_package", "package kind")
    require(package["authoritative_stage"] == "Base", "authoritative stage")
    require(package["root_id"] == "qk_gbfp8_head64_launch_harness_repair_static_v26_action_root", "root id")
...
def verify_launch_catalog() -> dict[str, Any]:
    ...
    require(catalog["artifact_kind"] == "qk_gbfp8_head64_launch_harness_repair_v26_launch_negative_catalog", "launch catalog kind")
...
def verify_v25_extension_mutations(package: dict[str, Any]) -> dict[str, Any]:
    ...
    for section in ("v23_preservation", "v24_preservation", "bytecode_free_verifier_contract", "literal_b_launch_contract", "mode_seal"):
        for relative in _scalar_paths(package[section]):
            ...
            try:
                ...
                raise VerificationError(f"fail-closed carried V25 extension mutation accepted: {path}")
    changed = deepcopy(package)
    ...
    try:
        ...
        raise VerificationError("fail-closed carried V25 fixture inventory mutation accepted")
    changed = deepcopy(package)
    ...
    try:
        ...
        raise VerificationError("fail-closed carried V25 launch catalog inventory mutation accepted")
...
def verify_v26_extension_mutations(package: dict[str, Any]) -> dict[str, Any]:
    rejected = []
    for relative in _scalar_paths(package["v25_preservation"]):
        changed = deepcopy(package)
        path = ("v25_preservation",) + relative
        original: Any = changed
        for part in path:
            original = original[part]
        _set_path(changed, list(path), _mutated_scalar(original))
        try:
            ...
            raise VerificationError(f"fail-closed V26 preservation mutation accepted: {path}")
    return {"rejected_mutation_count": len(rejected), "rejected_mutation_paths": rejected}
...
def build_negative_report(package: dict[str, Any]) -> dict[str, Any]:
    ...
    report = {
        "artifact_kind": "qk_gbfp8_head64_launch_harness_repair_v26_synthetic_negative_fixture_report",
        ...
        "status": "PASS_V26_LITERAL_B_MODE_SEAL_AND_SYNTHETIC_MUTATION_REJECTION",
    }
...
def verify(allow_acceptance: bool = False, launch_proof: dict[str, Any] | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    report = {
        "artifact_kind": "qk_gbfp8_head64_launch_harness_repair_static_v26_candidate_verifier_report",
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=BUILD_ROOT / "QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_CANDIDATE_REPORT.json")
    parser.add_argument("--negative-output", type=Path, default=BUILD_ROOT / "QK_GBFP8_HEAD64_LAUNCH_HARNESS_REPAIR_STATIC_V26_NEGATIVE_FIXTURE_REPORT.json")
