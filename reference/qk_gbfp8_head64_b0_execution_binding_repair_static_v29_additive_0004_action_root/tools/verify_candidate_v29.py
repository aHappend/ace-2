...
def _source_for(records: list[dict[str, Any]], raw: bytes, index: int, role: str, info: Any = FakeInfo) -> dict[str, Any]:
...
def _capsules(package: dict[str, Any], now_ts: float = 1000.0, info: Any = FakeInfo) -> tuple[bytes, dict[str, Any], dict[str, Any]]:
    nonce = "a" * 64
    ...
    manager = _rehash(
        {
            ...
            "role": "MANAGER",
            "schema_version": 1,
            "source": _source_for(records, raw, 0, "MANAGER", info),
        },
        "capsule_sha256",
    )
    operator = _rehash(
        {
            ...
            "role": "OPERATOR",
            "schema_version": 1,
            "source": _source_for(records, raw, 1, "OPERATOR", info),
        },
        "capsule_sha256",
    )
...
def _run_real_pathname_races(rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0004-races-", dir="/tmp") as temporary:
        root = Path(temporary)
        ...
        expected_raw = move_source.read_bytes()
        ...
        try:
            _expect_contract_code(
                ...
                lambda: W._move_consumed_inode(move_source, move_destination, opened, expected_raw, "OPERATOR"),
                rejected,
            )
        finally:
            W.os.rename = original_rename

        rewrite_source = root / "rewrite-source.json"
        rewrite_destination = root / "rewrite-consumed.json"
        rewrite_source.write_bytes(C.compact_bytes({"generation": "opened"}))
        rewrite_raw = rewrite_source.read_bytes()
        rewritten_raw = rewrite_raw.replace(b"opened", b"mutate")
        C.require(len(rewritten_raw) == len(rewrite_raw), "CAPSULE_REWRITE_FIXTURE_SIZE", str(len(rewritten_raw)))
        rewrite_opened = os.lstat(rewrite_source)

        def rename_after_same_inode_rewrite(source: Any, destination: Any) -> None:
            with rewrite_source.open("r+b") as handle:
                handle.seek(0)
                handle.write(rewritten_raw)
                handle.truncate()
                handle.flush()
                os.fsync(handle.fileno())
            os.utime(rewrite_source, ns=(rewrite_opened.st_atime_ns, rewrite_opened.st_mtime_ns))
            C.require(os.lstat(rewrite_source).st_ino == rewrite_opened.st_ino, "CAPSULE_REWRITE_INODE_CHANGED", str(rewrite_source))
            original_rename(source, destination)

        W.os.rename = rename_after_same_inode_rewrite
        try:
            _expect_contract_code(
                "capsule_same_inode_rewrite_before_final_move_real_race",
                "CAPSULE_MOVED_BYTES_MISMATCH",
                lambda: W._move_consumed_inode(rewrite_source, rewrite_destination, rewrite_opened, rewrite_raw, "OPERATOR"),
                rejected,
            )
        finally:
            W.os.rename = original_rename

        metadata_source = root / "metadata-source.json"
        metadata_destination = root / "metadata-consumed.json"
        metadata_source.write_bytes(C.compact_bytes({"generation": "metadata"}))
        metadata_raw = metadata_source.read_bytes()
        metadata_opened = os.lstat(metadata_source)

        def rename_after_metadata_change(source: Any, destination: Any) -> None:
            os.chmod(metadata_source, stat.S_IMODE(metadata_opened.st_mode) ^ 0o100)
            original_rename(source, destination)

        W.os.rename = rename_after_metadata_change
        try:
            _expect_contract_code(
                "capsule_metadata_change_before_final_move_real_race",
                "CAPSULE_MOVED_METADATA_MISMATCH",
                lambda: W._move_consumed_inode(metadata_source, metadata_destination, metadata_opened, metadata_raw, "MANAGER"),
                rejected,
            )
        finally:
            W.os.rename = original_rename
...
def _fixture_consumed_capsule(
    source: Path,
    destination: Path,
    role: str,
    package: dict[str, Any],
) -> dict[str, Any]:
    value, raw, validated_stat = W._read_stable_capsule(source)
    proof = W.C.validate_authority_capsule(value, role, package, 1100.0)
    W._move_consumed_inode(source, destination, validated_stat, raw, role)
    return {
        "consumed_path": destination,
        "proof": proof,
        "raw": raw,
        "role": role,
        "validated_stat": validated_stat,
        "value": value,
    }


def _run_post_consumption_event_races(package: dict[str, Any], rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0004-post-consumption-", dir="/tmp") as temporary:
        root = Path(temporary)
        runtime = root / "runtime"
        failure_seal = root / "runtime.failed-start"
        event_log = root / "events.jsonl"
        manager_source = root / "manager-admission.json"
        operator_source = root / "operator-authority.json"
        original_event_log = C.EVENT_LOG
        original_wrapper_event_log = W.C.EVENT_LOG
        ...
        def forbidden_process_start(*args: Any, **kwargs: Any) -> Any:
            ...
            process_starts += 1
            raise AssertionError("protected process start reached after post-consumption event drift")

        C.EVENT_LOG = event_log
        W.C.EVENT_LOG = event_log
        W.C.RUNTIME_NAMESPACE = runtime
        W.C.FAILURE_SEAL_NAMESPACE = failure_seal
        W.subprocess.run = forbidden_process_start
        claim_token = "c" * 64
        try:
            raw, _, _ = _capsules(package)
            event_log.write_bytes(raw)
            raw, manager_value, operator_value = _capsules(package, info=os.lstat(event_log))
            event_log.write_bytes(raw)
            manager_source.write_bytes(C.compact_bytes(manager_value))
            operator_source.write_bytes(C.compact_bytes(operator_value))

            W._claim_absent_runtime(runtime)
            W._initialize_claimed_runtime(runtime, claim_token)
            manager = _fixture_consumed_capsule(
                manager_source,
                runtime / "authority/consumed/manager-admission.json",
                "MANAGER",
                package,
            )
            operator = _fixture_consumed_capsule(
                operator_source,
                runtime / "authority/consumed/operator-authority.json",
                "OPERATOR",
                package,
            )
            envelope = root / "frozen-envelope.json"
            W._atomic_write(envelope, C.compact_bytes({"fixture": True}))
            ...
            event_log.write_bytes(raw + unrelated_event)
            _expect_contract_code(
                "post_consumption_event_append_rejected_before_protected_start",
                "EVENT_LIVE_FRONTIER_DRIFT",
                lambda: W._protected_boundary(runtime, claim_token, envelope, package, manager, operator),
                rejected,
            )
            C.require(process_starts == 0, "POST_CONSUMPTION_APPEND_PROTECTED_PROCESS_STARTED", str(process_starts))

            event_log.write_bytes(raw[:-1])
            _expect_contract_code(
                "post_consumption_event_truncation_rejected_before_envelope_publication",
                "EVENT_LOG_TRUNCATED_OR_FRONTIER",
                lambda: W._write_frozen_envelope(runtime, claim_token, package, {}, manager, operator),
                rejected,
            )
            C.require(
                not os.path.lexists(runtime / "authority/internal/frozen-envelope.json"),
                "POST_CONSUMPTION_TRUNCATION_ENVELOPE_PUBLISHED",
                str(runtime),
            )

            replacement = root / "events-replacement.jsonl"
            replacement.write_bytes(raw)
            os.replace(replacement, event_log)
            _expect_contract_code(
                "post_consumption_event_replacement_rejected_before_protected_start",
                "EVENT_LOG_IDENTITY_MISMATCH",
                lambda: W._protected_boundary(runtime, claim_token, envelope, package, manager, operator),
                rejected,
            )
            C.require(process_starts == 0, "POST_CONSUMPTION_REPLACEMENT_PROTECTED_PROCESS_STARTED", str(process_starts))
        finally:
            W.subprocess.run = original_run
            W.C.FAILURE_SEAL_NAMESPACE = original_failure_seal
            W.C.RUNTIME_NAMESPACE = original_runtime
            W.C.EVENT_LOG = original_wrapper_event_log
            C.EVENT_LOG = original_event_log


def _run_real_atomic_duplicate_start(rejected: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="v29-additive-0004-duplicate-start-", dir="/tmp") as temporary:
        root = Path(temporary)
        runtime = root / "runtime"
        failure_seal = root / "runtime.failed-start"
        original_runtime = W.C.RUNTIME_NAMESPACE
...
def run_regressions(package: dict[str, Any]) -> dict[str, Any]:
    ...
    _run_post_consumption_event_races(package, rejected)
