#!/usr/bin/env python3
"""Phase-safe fixed-input-scale launcher for nonofficial hybrid attempt 0025."""
...
PROMPTS = PRIVATE / "prompts-0025.json"
SELECTION = PRIVATE / "candidate-selection-0025.json"
CANDIDATE_BUDGET_REGRESSION = PRIVATE / "candidate-budget-regression-0025.json"
TOKENIZATION_PREFLIGHT = PRIVATE / "exact-tokenization-preflight-0025.json"
TOKENIZATION_REGRESSION = PRIVATE / "tokenization-verifier-regression-0025.json"
INPUT_SCALE_REGRESSION = PRIVATE / "fixed-input-scale-regression-0025.json"
SCALE_REGRESSION = PRIVATE / "retained-scale-regression-0025.json"
LIFECYCLE_REGRESSION = PRIVATE / "record-lifecycle-regression-0025.json"
GATE_ORDER_REGRESSION = PRIVATE / "gate-order-regression-0025.json"
TEMPLATE_GATE = PRIVATE / "package-template-invariant-0025.json"
DEPENDENCY_PROBE = PRIVATE / "dependency-probe-0025.json"
LAUNCH_REGRESSION = PRIVATE / "launch-fidelity-regression-0025.json"
ARGV_SHAPE_REGRESSION = PRIVATE / "argv-shape-regression-0025.json"
CLOSURE_CERTIFICATE = PRIVATE / "closure-certificate-0025.json"
PHASE_TRANSITION_REGRESSION = PRIVATE / "phase-transition-regression-0025.json"
INVOCATION_REGRESSION = PRIVATE / "invocation-fidelity-regression-0025.json"
ORCHESTRATION_REGRESSION = PRIVATE / "orchestration-regression-0025.json"
LONG_PATH_REGRESSION = PRIVATE / "long-path-transport-regression-0025.json"
LONG_PATH_REGRESSION_SOURCE = (
...
SEALED_0024_TERMINAL = SEALED_0024_OUTPUT / "terminal-record.json"
SEALED_0024_LAUNCHER = (
    ROOT / "tools/run_stage1_layer23_v_rank1_hybrid_0024.py"
)
SEALED_0024_PROMPTS = PRIVATE / "prompts-0024.json"
SEALED_0024_SELECTION = PRIVATE / "candidate-selection-0024.json"
...
SEALED_0024_TERMINAL_SHA256 = (
    "49909775546d31fdd7eb49bde47763435a0c7859404ad8dcccaa1ff971a3ee63"
)
SEALED_0024_LAUNCHER_SHA256 = (
    "84f2fe8b7df35193f1a21988a233ddec41891a9bc1e0f8b36b0c3a8945ddd4c0"
)
SEALED_0024_PROMPTS_SHA256 = (
    "79173dae4a68723ffc4de2dd01ce2e77683de12f0dbae688a5d8c61b2d4ce685"
)
SEALED_0024_SELECTION_SHA256 = (
    "8069b1b43b4de42892bd52d54f24ab3fef307c738e17c077bd069baa1b9f6b7b"
)

EXPECTED_DENY_ID_COUNT = 44
EXPECTED_DENY_HASH_COUNT = 54
EXPECTED_DENY_ID_DIGEST = (
    "3e8f4edc598f42131d1fa664b37261902122dd51100f65c5cd24fe508b7be5f1"
)
EXPECTED_DENY_HASH_DIGEST = (
    "cc55e4441c92da0a75b2e10f1a226d3c2528459d6680969b5c65efdd50c33e51"
)

CANDIDATES = (
    {"id": "fresh-0025-a", "text": "Why do comets glow?"},
    {"id": "fresh-0025-b", "text": "Where do badgers burrow?"},
)
EXPECTED_SELECTED_CANDIDATES = (
    {
        **CANDIDATES[0],
        "sha256": "72ee328de8431cc45f5dc966117df78add8fe30ac171aa0bf8d193ff85d80bd1",
        "prompt_token_count": 35,
        "total_context_token_count": 39,
    },
    {
        **CANDIDATES[1],
        "sha256": "22fce0357cb01ad7646be6cc28112e934034453aadb3ec467ec60d0ac16c7602",
        "prompt_token_count": 36,
        "total_context_token_count": 40,
    },
)
OVER_BUDGET_CANDIDATE = {
    "id": "fresh-0025-over-budget",
...
REUSED_HASH_CANDIDATE = {
    "id": "fresh-0025-hash-reuse",
    "text": "Where do otters sleep?",
}
...
def _verify_long_path_regression() -> dict[str, Any]:
    _require(
        stat.S_ISREG(os.lstat(LONG_PATH_REGRESSION).st_mode)
        and stat.S_ISREG(os.lstat(LONG_PATH_REGRESSION_SOURCE).st_mode),
        "synthetic long-path regression source or record is absent",
    )
    record = json.loads(LONG_PATH_REGRESSION.read_text(encoding="utf-8"))
    _require(
        record.get("status") == "PASS_SYNTHETIC_LONG_PLUSARG_PATH_REGRESSION"
        ...
        "synthetic long-path regression changed or is incomplete",
    )
    return record
...
def _verify_0024_stdlib_seal() -> dict[str, Any]:
    _verify_long_path_regression()
    paths = (
        SEALED_0024_TERMINAL,
        SEALED_0024_LAUNCHER,
        SEALED_0024_PROMPTS,
        SEALED_0024_SELECTION,
    )
    for path in paths:
        _require(
            stat.S_ISREG(os.lstat(path).st_mode),
            f"sealed 0024 artifact is not regular: {path}",
        )
    terminal = json.loads(SEALED_0024_TERMINAL.read_text(encoding="utf-8"))
    prompts = json.loads(SEALED_0024_PROMPTS.read_text(encoding="utf-8"))
    selection = json.loads(SEALED_0024_SELECTION.read_text(encoding="utf-8"))
    selected_prompts = [
        {"id": item["id"], "text": item["text"]} for item in selection["selected"]
    ]
    _require(
        _sha256_file(SEALED_0024_TERMINAL) == SEALED_0024_TERMINAL_SHA256
        and _sha256_file(SEALED_0024_LAUNCHER) == SEALED_0024_LAUNCHER_SHA256
        and _sha256_file(SEALED_0024_PROMPTS) == SEALED_0024_PROMPTS_SHA256
        and _sha256_file(SEALED_0024_SELECTION) == SEALED_0024_SELECTION_SHA256
        ...
        and terminal.get("process", {}).get("returncode") == 1
        and terminal.get("process", {}).get("wall_time_seconds")
        == 652.381338809
        and selection.get("attempt_identity") == "nonofficial-hybrid-0024"
        and selection.get("status") == "PASS_NONCONSUMING_CANDIDATE_SELECTION"
        and selection.get("deny_set", {}).get("combined_id_count") == 42
        and selection.get("deny_set", {}).get("combined_hash_count") == 52
        and prompts == {"schema_version": 1, "prompts": selected_prompts},
        "sealed 0024 terminal, launcher, prompts, or selection changed",
    )
    return {
        "terminal_sha256": SEALED_0024_TERMINAL_SHA256,
        "launcher_sha256": SEALED_0024_LAUNCHER_SHA256,
        "prompts_sha256": SEALED_0024_PROMPTS_SHA256,
        "selection_sha256": SEALED_0024_SELECTION_SHA256,
...
def _invocation_regression_record(
    ...
    checks = {
        ...
        "sealed_0024_exact_once_failure_preserved": (
            _verify_0024_stdlib_seal()["process_start_count"] == 1
        ),
        "process_not_started": True,
    }
    return {
        "schema_version": 1,
        "mission_id": "stage1rank1hybrid25execute",
        "attempt_identity": "nonofficial-hybrid-0025",
        ...
        "sealed_0024": _verify_0024_stdlib_seal(),
        "checks": checks,
    }
...
def _verify_invocation_regression() -> dict[str, Any]:
    _require(
        ...
        "0025 invocation-fidelity regression is absent or not regular",
    )
    ...
    _require(
        ...
        and record.get("mission_id") == "stage1rank1hybrid25execute"
        and record.get("attempt_identity") == "nonofficial-hybrid-0025"
        ...
        and record.get("sealed_0024") == _verify_0024_stdlib_seal()
        and record.get("checks")
        and all(record["checks"].values()),
        "0025 invocation-fidelity regression changed or is incomplete",
    )
    return record
...
def _run_invocation_regression(inherited_snapshot: Path) -> int:
    ...
    _require(
        ...
        "0025 invocation or execution namespace already exists",
    )
    ...
    _require(
        ...
        "0025 invocation-fidelity regression failed",
    )
    ...
    print(
        "ACE2_HYBRID_0025_INVOCATION_FIDELITY_REGRESSION_PASS "
        "exact_env_i=true inherited_rejected=true mutations_rejected=3 "
        "sealed_0024_process_start_count=1 process_start_count=0",
        flush=True,
    )
    return 0
...
def _seal_preloader_failure(error: BaseException) -> None:
    ...
    record = {
        ...
        "failure": {
            ...
            "regression": "No retry, replay, or resume; preserve all 0025 artifacts.",
        },
    }
    try:
...
def _diagnostic_files(prior_0014: Any) -> list[Path]:
    ...
    _require(
        ...
        "0025 diagnostic prompt source set is incomplete",
    )
    return files
...
def _run_orchestration_regression() -> int:
    ...
    _require(
        ...
        "0025 orchestration regression already exists",
    )
    _require(
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-0025-duplicate-fixture-") as temporary:
    ...
    _require(all(checks.values()), "0025 orchestration regression failed")
    ...
    with ORCHESTRATION_REGRESSION.open("xb") as stream:
        ...
        os.fsync(stream.fileno())
    print(
        "ACE2_HYBRID_0025_ORCHESTRATION_REGRESSION_PASS "
...
def _configure_0025(
    ...
    runner.MISSION_ID = "stage1rank1hybrid25execute"
    runner.ATTEMPT_ID = "nonofficial-hybrid-0025"
    runner.PREDECESSOR_TERMINAL_SEALS = {
        **runner.PREDECESSOR_TERMINAL_SEALS,
        "nonofficial-hybrid-0024": {
            "sha256": SEALED_0024_TERMINAL_SHA256,
            "process_start_count": 1,
            "process_started": True,
        },
    }
    ...
    def verify_predecessor_seals_0025() -> None:
        prior_verify_predecessor_seals()
        _verify_0024_stdlib_seal()

    runner.verify_predecessor_seals = verify_predecessor_seals_0025

    def verify_exact_interpreter_0025() -> None:
        prior_verify_exact_interpreter()
        _verify_invocation_regression()

    runner.verify_exact_interpreter = verify_exact_interpreter_0025
    ...
    def expected_candidate_budget_regression() -> dict[str, Any]:
        ...
        added_id_set = {*prompt_ids, "fresh-0025-addition-probe"}
        ...
        checks = {
            ...
            "sealed_0024_exact_once_failure_bound": (
                _verify_0024_stdlib_seal()["process_start_count"] == 1
            ),
            "process_not_started": True,
        }
    ...
    def verify_candidate_budget_regression() -> dict[str, Any]:
        runner.require(
            ...
            "0025 candidate-budget regression is absent or not regular",
        )
        record = runner.read_json(CANDIDATE_BUDGET_REGRESSION)
        runner.require(
            record == expected_candidate_budget_regression()
            and all(record["checks"].values()),
            "0025 candidate-budget regression changed",
        )
        return record
    ...
    def candidate_budget_regression() -> int:
        ...
        runner.require(
            ...
            "0025 candidate-budget regression already exists",
        )
        record = expected_candidate_budget_regression()
        runner.require(
            all(record["checks"].values()),
            "0025 candidate-budget regression failed",
        )
        runner.write_json(CANDIDATE_BUDGET_REGRESSION, record)
        verify_candidate_budget_regression()
        print(
            "ACE2_HYBRID_0025_CANDIDATE_BUDGET_REGRESSION_PASS "
            "deny_ids=44 deny_hashes=54 deletion_addition_reuse_rejected=true "
    ...
    sealed_0024_paths = (
        ...
        SEALED_0024_PROMPTS,
        SEALED_0024_SELECTION,
        LONG_PATH_REGRESSION,
        LONG_PATH_REGRESSION_SOURCE,
    )

    def source_records_0025(
        model: Path,
        tokenizer_json: Path,
        ...
        for path in sealed_0024_paths:
            records[runner.public_path(path)] = runner.file_record(path)
        return dict(sorted(records.items()))

    runner.source_records = source_records_0025

    def execution_bound_paths_0025(
        prompts_path: Path,
        model: Path,
        tokenizer_json: Path,
        tokenizer_config: Path,
        ...
        return sorted(
            {
                ...
                *(path.resolve(strict=True) for path in sealed_0024_paths),
            },
            key=lambda item: str(item),
        )

    runner.execution_bound_paths = execution_bound_paths_0025

    def freeze_0025(prompts_path: Path) -> int:
        result = prior_freeze(prompts_path)
        frozen = runner.read_json(runner.FREEZE)
        frozen["sealed_0024"] = {
            "terminal": runner.file_record(SEALED_0024_TERMINAL),
            "launcher": runner.file_record(SEALED_0024_LAUNCHER),
            "private_prompts": runner.file_record(
                SEALED_0024_PROMPTS, expose_path=False
            ),
            "selection": runner.file_record(SEALED_0024_SELECTION),
            "process_start_count": 1,
            "process_started": True,
            "outcome": "child_nonzero_exit",
        }
    ...
    runner.freeze = freeze_0025

    def execute_0025(prompts_path: Path, expected_freeze_sha256: str) -> int:
    ...
    def verify_0025() -> int:
        ...
        runner.require(
            ...
            .get("package", {})
            .get("package_id")
            == "stage1rank1hybrid25execute-nonofficial-hybrid-0025"
            and attestation.get("attempt_identity") == "nonofficial-hybrid-0025"
            and attestation.get("mission_id") == "stage1rank1hybrid25execute"
            ...
            "0025 terminal or exact deny-set binding changed",
        )
        print(
            "ACE2_HYBRID_0025_DECISIVE_VERIFY_PASS "
            "prompt_count=2 generated_tokens_per_prompt=4 rtl_v_bytes=1024 "
            "deny_ids=44 deny_hashes=54 process_start_count=1 "
...
def _patch_template(template: Any) -> None:
    ...
    def patch_inherited_chain(prior: Any, prior_0014: Any) -> None:
        ...
        prior_0014.SEALED_BOUNDARY_SHA256 = {
            **prior_0014.SEALED_BOUNDARY_SHA256,
            str(SEALED_0024_TERMINAL.relative_to(ROOT)): (
                SEALED_0024_TERMINAL_SHA256
            ),
            str(SEALED_0024_LAUNCHER.relative_to(ROOT)): (
                SEALED_0024_LAUNCHER_SHA256
            ),
            str(SEALED_0024_PROMPTS.relative_to(ROOT)): SEALED_0024_PROMPTS_SHA256,
            str(SEALED_0024_SELECTION.relative_to(ROOT)): (
                SEALED_0024_SELECTION_SHA256
            ),
        }
    ...
    def configure_runner(
        ...
        _require("prior_0014" in configured, "0025 inherited chain was not configured")
