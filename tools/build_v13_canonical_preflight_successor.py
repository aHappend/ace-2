#!/usr/bin/env python3
"""Build the immutable Base V13 canonical-byte read-only-preflight successor."""
...
V13_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree_action_root"
V8_STATIC_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_static_v8_action_root"
VERIFIER_SOURCE = ROOT / "tools/v13_canonical_preflight_verify_independent.py"
...
PACKAGE_PATH = V13_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_PACKAGE.json"
LAUNCHER_PATH = V13_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v13.py"
TRANSPORT_PATH = V13_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v13.py"
STATIC_VERIFIER_PATH = V13_ROOT / "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree.py"
REPORT_PATH = V13_ROOT / "V9_CANONICAL_BYTE_FAILURE_REPORT.md"
CANONICAL_PACKAGE = V13_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_PACKAGE.canonical.json"
CANONICAL_RESULT_SCHEMA = V13_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_RESULT_SCHEMA.canonical.json"
...
def replace_v9(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("V9", "V13").replace("v9", "v13").replace(
            "ace2:qk-gbfp8-base-v13:execute-once:2254dd91:20260813T2013Z",
            ACTION_ID,
        )
...
def main() -> int:
    require(not V13_ROOT.exists(), f"refusing to overwrite successor root: {V13_ROOT}")
    ...
    v13_schema_dir = V13_ROOT / "reference"
    schema_names = (
        "AUTHORITY_SCHEMA",
        "CREDENTIAL_SCHEMA",
    ...
    for name in schema_names:
        ...
        target = v13_schema_dir / f"QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_{name}.json"
        schema = replace_v9(strict_object(source.read_bytes()))
        if name == "FRESH_L2_ACCEPTANCE_SCHEMA":
            schema["properties"].update({
                "independent_verifier_rerun_sha256": {"pattern": "^[0-9a-f]{64}$", "type": "string"},
                "reviewer_decision_sha256": {"pattern": "^[0-9a-f]{64}$", "type": "string"},
                "reviewer_thread_id": {"minLength": 1, "type": "string"},
                "target_process_starts": {"const": 0},
            })
    ...
    report = f"""# Base V9 canonical-byte preflight failure and V13 additive repair
...
"""
    ...
    launcher = v9_launcher.replace("V9", "V13").replace("v9", "v13")
    launcher = launcher.replace(
        'ACTION_ID = "ace2:qk-gbfp8-base-v13:execute-once:2254dd91:20260813T2013Z"',
...
'''
    ...
    transport = v9_transport.replace("V9", "V13").replace("v9", "v13")
    ...
    transport_argv = [
        ...
        str(PACKAGE_PATH),
        "--acceptance",
        str(V13_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"),
    ...
    package["static_file_policy"] = {
        "allowed_relative_files": [
            "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_PACKAGE.json",
            ...
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_AUTHORITY_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_CREDENTIAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_FIRST_TERMINAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_LEDGER_SCHEMA.json",
            "review/FRESH_L2_STATIC_ACCEPTANCE.json",
            "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v13.py",
            "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v13.py",
            "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree.py",
        ],
    ...
    for path in sorted(V13_ROOT.rglob("*"), reverse=True):
        if path.is_file():
            os.chmod(path, 0o444)
    for path in sorted((item for item in V13_ROOT.rglob("*") if item.is_dir()), reverse=True):
        os.chmod(path, 0o555)
    os.chmod(V13_ROOT, 0o555)
    print(json.dumps({
        "action_id": ACTION_ID,
        ...
        "root": str(V13_ROOT),
        "status": "BUILT_IMMUTABLE_V13_CANONICAL_PREFLIGHT_SUCCESSOR_NO_AUTHORITY",
    }, sort_keys=True))
    return 0
