#!/usr/bin/env python3
"""Run focused RTL verification for the selected grouped layer-17 Q output."""
...
GENERATED = ROOT / "verification/generated/ace2_layer17_q_output_grouped_scale32"
RTL = ROOT / "rtl/ace2_layer17_q_output_grouped_scale32_core.sv"
TB = ROOT / "verification/tb/ace2_layer17_q_output_grouped_scale32_tb.sv"
SEARCH = ROOT / "tools/ace2_checkpoint176_layer17_q_output_grouped_scale32_search.py"
GENERATOR = ROOT / "tools/gen_layer17_q_output_grouped_scale32_vectors.py"
UNIT_TEST = ROOT / "verification/test_layer17_q_output_grouped_scale32_reference.py"
RUNNER = Path(__file__).resolve()
RTL_TOP = "ace2_layer17_q_output_grouped_scale32_core"


PASS_PATTERN = re.compile(
    r"ACE2_LAYER17_Q_OUTPUT_GROUPED_SCALE32_RTL_PASS "
...
def main() -> int:
    ...
    group_size = int(candidate_result["selected_q_output_group_size"])
    ...
    run(
        [
            ...
            "-m",
            "unittest",
            "verification.test_layer17_q_output_grouped_scale32_reference",
            "-v",
        ],
        unit_log,
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="ace2-layer17-q-output-grouped-", dir=ROOT / "build"
    ) as temporary:
        ...
        compile_stdout = run(
            [
                ...
                "-s",
                "ace2_layer17_q_output_grouped_scale32_tb",
                "-o",
                str(executable),
                str(RTL),
                str(TB),
    ...
    require(
        metrics["groups"] == manifest["shape"]["q_output_groups"],
        "model group count differs",
    )
    require(
        metrics["saturations"] == manifest["expected"]["q_saturation_count"],
    ...
    result = {
        ...
        "status": "PASS_FOCUSED_LAYER17_Q_OUTPUT_GROUPED_SCALE32_RTL_REFERENCE_AGREEMENT",
        ...
        "claim_boundary": (
            "Frozen candidate rank selection plus focused synthesizable RTL/reference "
            "agreement only; no independent acceptance, official generation, U280/Stage 2, "
    ...
    print(
        "ACE2_LAYER17_Q_OUTPUT_GROUPED_SCALE32_RTL_RESULT "
        f"status={result['status']} group={group_size} "
        f"rank={result['selected_candidate']['reference_rank']} cycles={metrics['cycles']}"
    )
    return 0
