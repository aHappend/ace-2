...
def inventory(root: Path, *, exclude: set[str] | None = None) -> dict[str, dict[str, int | str]]:
...
def verify_pre_authority() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    for name in (
        "authorization.json",
        "package-checks.json",
        "reproducibility-report.json",
        "product_driver.py",
        "launch.sh",
        "Vace2_shell_runtime_harness",
    ):
        core.require_file(PACKAGE / name, core.PRE_AUTHORITY_HASHES[name], f"pre-authority {name}")
    core._validate_manager_mission()
    ...
    return (
        ...
        core.load_object(PACKAGE / "reproducibility-report.json"),
    )
...
def materialize_authority() -> None:
    ...
    if core.AUTHORITY_RECORD.exists():
        if core.AUTHORITY_RECORD.read_bytes() != raw:
            raise core.ProductError("existing CF09 execution authority differs")
    else:
...
def copy_package(destination: Path) -> None:
    shutil.copytree(
        ...
        ignore=shutil.ignore_patterns("__pycache__", "*.pyc", ".cf09-authority-*"),
    )
...
def main() -> int:
    ...
    base_authorization, base_checks, _ = verify_pre_authority()
    materialize_authority()
    normalize_source_modes()
    gates = core.validate_prerequisites()
    with tempfile.TemporaryDirectory(prefix=".cf09-authority-build-a-", dir=PACKAGE.parent) as first:
        with tempfile.TemporaryDirectory(prefix=".cf09-authority-build-b-", dir=PACKAGE.parent) as second:
            ...
            first_inventory = inventory(first_path)
            second_inventory = inventory(second_path)
            if first_inventory != second_inventory:
                raise core.ProductError("CF09 authority builds differ by byte or mode")
            for name in METADATA:
    ...
    core.assert_fresh_execution_state()
    print("PASS_CF09_AUTHORITY_TWO_BUILD_BYTE_MODE_EQUAL_NON_EXECUTING")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"CF09 authority construction refused: {error}", file=sys.stderr)
        raise SystemExit(2)
