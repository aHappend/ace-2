#!/usr/bin/env python3
"""Freeze or reproduce the marker-free V8 full-finetune execution package."""
...
DATASET = ROOT / "research/training/qwen2.5-0.5b-instruct-bf16-full-finetune-product-v8"
PACKAGE = ROOT / "pilot/qwen25_05b_bf16_full_finetune_product_v8"
RUNNER = ROOT / "pilot/qwen25_05b_bf16_full_finetune_product_v8_runner"
OFFLINE = ROOT / "build/offline-preflight/qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8"
...
SOURCE_DATASET = ROOT / "research/training/qwen2.5-0.5b-instruct-bf16-dora-product-v6"
SOURCE_DATASET_MANIFEST = SOURCE_DATASET / "dataset_manifest.json"
...
MANIFEST = OFFLINE / "v8-execution-package-manifest.json"
SELF_TEST = OFFLINE / "v8-execution-package-self-test.json"
...
def assert_marker_free() -> None:
    if RUN_ROOT.exists():
        raise RuntimeError("official V8 namespace exists")
    if AUTHORITY.exists():
        raise RuntimeError("V8 attempt authority exists")
    for name in ("v8-detached-launch-intent.json", "v8-detached-process.json", "v8-launch-terminal-status.json", "v8-worker-terminal-status.json"):
        if (OFFLINE / name).exists():
            raise RuntimeError(f"V8 execution record exists: {name}")
    for name in ("patch.jsonl", "synthetic_probes.jsonl", "train.jsonl"):
        if sha256(DATASET / name) != sha256(SOURCE_DATASET / name):
            raise RuntimeError(f"V8 immutable source-data copy differs: {name}")


def environment_lock() -> dict[str, Any]:
    return {
        "contract_id": "qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8",
        "cuda_runtime": "13.0",
        "environment": {
...
def dataset_manifest() -> dict[str, Any]:
    return {
        "claim_boundary": "Fresh V8 identity over byte-identical immutable pre-V7 training/probe inputs and protected evaluator bindings only; no V7 checkpoint, V7 output, official evaluator record, or model execution was opened or reused.",
        "construction": {
            "dev_or_holdout_answer_fields_accessed": False,
            "dev_or_holdout_records_parsed": False,
            "external_model_outputs_used": False,
            "immutable_v6_training_inputs_copied_exactly": True,
            "teacher_outputs_used": False,
            "v7_checkpoint_or_output_used": False,
        },
        "counts": {"additive_patch": 72, "synthetic_probes": 28, "train": 352, "v1_backbone": 280},
        "dataset_id": "qwen2.5-0.5b-instruct-bf16-full-finetune-product-v8",
        ...
        "integrity": {
            ...
            "source_dataset": {
                "manifest": binding(SOURCE_DATASET_MANIFEST),
                "path": SOURCE_DATASET.relative_to(ROOT).as_posix(),
                "patch_sha256": sha256(SOURCE_DATASET / "patch.jsonl"),
                "probe_sha256": sha256(SOURCE_DATASET / "synthetic_probes.jsonl"),
                "train_sha256": sha256(SOURCE_DATASET / "train.jsonl"),
            },
...
def training_recipe() -> dict[str, Any]:
    ...
    return {
        "attempt_policy": {
            ...
            "attempt_namespace_template": "build/qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8/attempt-0001",
        ...
        "early_stop": {
            ...
            "probe_path": "research/training/qwen2.5-0.5b-instruct-bf16-full-finetune-product-v8/synthetic_probes.jsonl",
            "probe_sha256_bound_before_training": True,
            "selection_inputs": ["fresh_disjoint_synthetic_probes"],
            "stop_at_first_epoch_satisfying_every_probe_gate": True,
            "training_loss_selects_checkpoint": False,
        },
        "environment_lock": {"path": "research/training/qwen2.5-0.5b-instruct-bf16-full-finetune-product-v8/environment.lock.json"},
        "full_finetune": {
        ...
        "namespace": {
            "model_root": "build/qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8",
            "package": "pilot/qwen25_05b_bf16_full_finetune_product_v8",
            "selected_bf16": "build/qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8/selected/bf16",
        },
        "optimizer": {
            "adam_beta1": 0.9,
            "adam_beta2": 0.95,
            ...
            "learning_rate": 5e-6,
            "learning_rate_justification": "Conservative source-model full-parameter rate frozen without consulting any V7 checkpoint, dev output, or quality result; it is not a measured optimum.",
...
def freeze_contract() -> dict[str, Any]:
    canonical_dev_policy = load(SOURCE_CONTRACT)["acceptance"]["dev"]
    return {
        "acceptance": {
            "dev": canonical_dev_policy,
            "generation": {"do_sample": False, "eos_token_ids": [151643, 151645], "num_beams": 1, "primary_max_new_tokens": 128},
        ...
        "claim_boundary": "Freeze-only V8 full-parameter contract. No authority, launch record, namespace, marker, training, evaluator, checkpoint, quantization, RTL, U280, latency, or product-quality result is created.",
        "evaluator": {
        ...
        "material_difference": {
            ...
            "parameter_efficient_adapter_used": False,
            "predecessor_checkpoint_or_output_reuse_allowed": False,
            "source_data_start": binding(SOURCE_DATASET_MANIFEST),
            "superseded_v6_dora_contract": binding(V6_CONTRACT),
            "successor_identity": "qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8",
            "v7_checkpoint_dev_output_and_consumed_evidence_used_for_selection_or_validation": False,
        },
        "mechanism": training_recipe()["full_finetune"],
        "resource_assumptions": {
            "claim_boundary": "Conservative prerequisites, not measured V8 usage or latency.",
        ...
        "score_policy_schema": {
            "canonical_source": binding(SOURCE_CONTRACT),
            "category_gate_fields": ["minimum_hard_passes", "total"],
            "policy_fields_consumed_by_score_gates": ["category_gates", "critical_safety_failures_maximum", "minimum_hard_passes", "response_count"],
            "schema_id": "ace2_score_gates_v1",
            "self_test_requires_isolated_positive_and_negative_paths": True,
        },
...
def frozen_config() -> dict[str, Any]:
    return {
        "claim_boundary": "Non-executing V8 full-parameter package configuration; grants no attempt authority and creates no official namespace, launch record, or marker.",
        ...
        "model_namespace": "qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8",
        "package_id": "qwen25_05b_bf16_full_finetune_product_v8",
...
def execution_contract() -> dict[str, Any]:
    ...
    return {
        "attempt_authority": {
            ...
            "required_status_if_later_granted": "AUTHORIZE_SINGLE_V8_FULL_FINETUNE_ATTEMPT",
            "separate_fresh_operator_authority_required": True,
        },
        "claim_boundary": "Exact marker-free V8 full-finetune execution package only. No authority, launch, namespace, marker, training, evaluator, selected model, quantization, RTL, synthesis/PPA, U280, latency, quality, or completion claim.",
        "contract_id": "qwen2.5-0.5b-instruct-ace2-bf16-full-finetune-product-v8-execution-package-v1",
        "environment_resolution": {
            ...
            "pythonhashseed_conflict": {
                ...
                "resolution": "No conflict; the V8 recipe and environment lock select the same value.",
        ...
        "future_authorized_lifecycle_order": [
        ...
        "independent_execution_package_review": {"author_self_approval_allowed": False, "minimum_level": "L2", "path": L2.relative_to(ROOT).as_posix(), "required_before_any_authority_launch_namespace_or_marker": True, "required_status": "ACCEPTED_V8_FULL_FINETUNE_EXECUTION_PACKAGE_NO_ATTEMPT"},
        ...
        "schema_version": 2,
        "source_and_evaluator": {"source_contract": binding(SOURCE_CONTRACT), "source_dataset_manifest": binding(SOURCE_DATASET_MANIFEST), "frozen_scorer": binding(SCORER), "retention_amendment": binding(RETENTION)},
        "source_teacher_data_evaluator": {"source_contract": binding(SOURCE_CONTRACT), "source_dataset_manifest": binding(SOURCE_DATASET_MANIFEST), "teacher": recipe["teacher"], "dataset_manifest": binding(DATASET / "dataset_manifest.json"), "frozen_scorer": binding(SCORER), "retention_amendment": binding(RETENTION)},
        "stage": "v8_full_finetune_execution_package",
        "status": "FROZEN_MARKER_FREE_PENDING_FRESH_INDEPENDENT_L2",
        "supersession": {
            "dora_or_other_peft_continuation_permitted": False,
            "preserved_superseded_v6_contract": binding(V6_CONTRACT),
            "v7_eval_recovery_permitted": False,
            "v7_replay_resume_rescore_or_checkpoint_reuse_permitted": False,
        },
    }
...
def review_request() -> dict[str, Any]:
    ...
    if result.get("status") != "PASS" or result.get("official_namespace_exists") is not False:
        raise RuntimeError("V8 self-test has not passed marker-free")
    return {
        "acceptance_requested": "ACCEPTED_V8_FULL_FINETUNE_EXECUTION_PACKAGE_NO_ATTEMPT",
        "claim_boundary": "Independent L2 review of the exact fresh-identity marker-free full-parameter package, canonical scorer/policy schema, and detached-worker interlocks; no attempt authority or execution is requested.",
        "exact_package": {
            ...
            "source_dataset_manifest": binding(SOURCE_DATASET_MANIFEST),
...
def check() -> None:
    ...
    for path, value in expected.items():
        if path.read_bytes() != canonical(value):
            raise RuntimeError(f"frozen V8 artifact differs: {path}")
        companion = path.with_suffix(path.suffix + ".sha256")
        if companion.read_text(encoding="ascii").split() != [sha256(path), path.name]:
            raise RuntimeError(f"V8 sidecar differs: {path}")
    print("ACE2_FULL_FINETUNE_V8_FREEZE_REPRODUCES_NO_AUTHORITY_NO_LAUNCH_NO_NAMESPACE")
...
def main() -> None:
    ...
    if args.write_freeze:
        write_freeze()
        print(f"ACE2_FULL_FINETUNE_V8_FREEZE_WRITTEN contract={sha256(CONTRACT)}")
    elif args.write_review_request:
        write(REQUEST, review_request())
        print(f"ACE2_FULL_FINETUNE_V8_L2_REQUEST_WRITTEN tree={load(MANIFEST)['tree_sha256']}")
    else:
        check()
