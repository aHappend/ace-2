#!/usr/bin/env python3
"""Run/check one static-only Fresh-L2 review of the immutable V13 successor."""
...
ACTION_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree_action_root"
MANIFEST = ACTION_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_PACKAGE.json"
VERIFIER = ACTION_ROOT / "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree.py"
REPORT = ACTION_ROOT / "V9_CANONICAL_BYTE_FAILURE_REPORT.md"
ACCEPTANCE_SCHEMA = ACTION_ROOT / "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V13_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json"
ACCEPTANCE_DIR = ACTION_ROOT / "review"
ACCEPTANCE = ACCEPTANCE_DIR / "FRESH_L2_STATIC_ACCEPTANCE.json"
REVIEW_ROOT = ROOT / "build/v13-canonical-preflight-successor-fresh-l2-review"
...
DISPOSITION = "V13_CANONICAL_PREFLIGHT_SUCCESSOR_STATIC_PACKAGE_ACCEPTED_NO_EXECUTION_AUTHORITY"
MISSION_ID = "v13-canonical-preflight-successor-static-fresh-l2"
EXACT_ENV = {"LANG": "C", "LC_ALL": "C", "PYTHONHASHSEED": "0", "TZ": "UTC", "PATH": "/usr/bin:/bin", "PYTHONDONTWRITEBYTECODE": "1"}
RULINGS = (
    "v13.canonical-byte-root-cause",
    "v13.exact-production-reader-pass",
    "v13.read-only-preflight-before-live-create",
    "v13.adversarial-reader-and-manifest-rejection",
    "v13.new-action-and-retired-v9-no-replay",
    "v13.zero-target-starts-and-no-live-state",
    "v13.static-acceptance-schema-review-binding",
    "v13.sealed-immutable-file-set",
)
...
def package_inputs(*, acceptance_expected: bool) -> dict[str, Any]:
    ...
    require(not os.path.lexists(ACTION_ROOT / "live"), "V13 live state exists")
...
def rerun_verifier(*, acceptance_expected: bool) -> dict[str, Any]:
    ...
    require(payload.get("status") == "PASS_V13_CANONICAL_PREFLIGHT_SUCCESSOR_INDEPENDENT_STATIC_VERIFICATION", "verifier status differs")
    require(payload.get("adversarial_cases") == 35, "verifier adversarial count differs")
...
def create_acceptance(decision: dict[str, Any]) -> None:
    ...
    payload = {
        "action_id": ACTION_ID,
        "artifact_kind": "qk_gbfp8_head64_granularity_sweep_execution_v13_shellfree_fresh_l2_static_acceptance",
...
def main() -> int:
    if "--check" in sys.argv[1:]:
        check_existing()
        print("V13_CANONICAL_PREFLIGHT_SUCCESSOR_FRESH_L2_CHECK_PASS")
        return 0
    ...
    objective = "Independently Fresh-L2-review the exact immutable V13 canonical-byte read-only-preflight successor without granting or consuming execution authority."
    instruction = (
        ...
        "Confirm _read_accepted_package_and_result_schema is called inside _preflight before the first _durable_create in main; no target starts or V13 live state exist; V9 remains immutable READ_ONLY_PREFLIGHT_FAILED with zero invocations/payload opens and no replay. "
        "Adversarially inspect all 35 rejection cases, binding replacement, hash replacement, old-action substitution, live-directory permission, acceptance-schema omission, and target-start changes. "
        ...
        "Also confirm the failed static V10, V11, and V12 candidate roots remain present, immutable, and without live/review state; they are not accepted successors. "
        "Cover these exact rulings in the reason: "
        + ", ".join(RULINGS)
        + f". Return done only if the exact static V13 package deserves disposition {DISPOSITION}. A done verdict does not authorize execution and does not advance research/PIPELINE_STATE.json."
    )
    review = Reviewer(runner, memory_maintenance_enabled=False).evaluate(
        ...
        round_max=1,
        session_id=sid,
        main_summary=f"V13 manifest SHA-256 {sha256(MANIFEST)}; the bound inert verifier passes 35 adversarial cases, exact production canonical reads, preflight-before-live ordering, and zero target starts.",
        main_error=None,
        raw_evidence=json.dumps({"verified_inputs": verified, "independent_verifier_rerun": verifier_rerun}, indent=2, sort_keys=True),
        scope="v13_canonical_preflight_static_successor_fresh_l2_only",
        checkpoint_path="",
        escalate_hint="Any V13 transport/launcher execution remains separately operator-authorized and exactly once.",
    ...
    decision = {
        ...
        "claim_boundary": verified["claim_boundary"],
        "disposition": DISPOSITION if accepted else "V13_CANONICAL_PREFLIGHT_SUCCESSOR_STATIC_PACKAGE_REJECTED_NO_AUTHORITY",
        "independent_verifier_rerun_sha256": sha256(RERUN),
    ...
    if not accepted:
        seal_review_root()
        print(f"V13_CANONICAL_PREFLIGHT_SUCCESSOR_FRESH_L2_REJECTED status={raw.get('status')}")
        return 2
    ...
    seal_review_root()
    check_existing()
    print(f"V13_CANONICAL_PREFLIGHT_SUCCESSOR_FRESH_L2_DONE thread={raw.get('thread_id')} manifest={sha256(MANIFEST)} acceptance={sha256(ACCEPTANCE)}")
    return 0
