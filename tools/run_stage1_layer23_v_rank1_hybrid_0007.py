#!/usr/bin/env python3
"""Pristine-environment preloader for nonofficial hybrid attempt 0007."""
...
TOKENIZATION_PREFLIGHT = PRIVATE / "exact-tokenization-preflight-0007.json"
TOKENIZATION_REGRESSION = PRIVATE / "tokenization-verifier-regression-0007.json"
...
EXPECTED_TOKENIZATION_CHECKS = {
    ...
    "predecessor_0006_terminal_and_certificate_seals_immutable": True,
}
...
def _seal_preloader_failure(error: BaseException) -> None:
    ...
    record = {
        ...
        "attempt_identity": "nonofficial-hybrid-0007",
...
def _configure_runner(runner: Any, post_import_environment: dict[str, str]) -> None:
    ...
    runner.ATTEMPT_ID = "nonofficial-hybrid-0007"
    ...
    runner.DEFAULT_PROMPTS = PRIVATE / "prompts-0007.json"
    runner.TEMPLATE_GATE = PRIVATE / "package-template-invariant-0007.json"
    runner.DEPENDENCY_PROBE = PRIVATE / "dependency-probe-0007.json"
    ...
    def verify_predecessor_seals_0007() -> None:
    ...
    runner.verify_predecessor_seals = verify_predecessor_seals_0007

    def dependency_probe_0007() -> int:
        runner.require(
    ...
    runner.dependency_probe = dependency_probe_0007
    ...
    def tokenization_preflight_0007(prompts_path: Path) -> int:
        runner.require(
            not TOKENIZATION_PREFLIGHT.exists(),
            "exact-tokenization preflight already exists",
        )
    ...
    runner.tokenization_preflight = tokenization_preflight_0007
    ...
    def tokenization_verifier_regression_0007(prompts_path: Path) -> int:
        runner.require(
            not TOKENIZATION_REGRESSION.exists(),
            "tokenization-verifier regression already exists",
        )
    ...
    runner.tokenization_verifier_regression = tokenization_verifier_regression_0007
    ...
    runner.source_records = source_records_0007

    def execution_bound_paths_0007(
        prompts_path: Path,
        model: Path,
        tokenizer_json: Path,
        tokenizer_config: Path,
    ...
    runner.execution_bound_paths = execution_bound_paths_0007

    def package_template_invariant_0007(prompts_path: Path) -> int:
        certificate = verify_tokenization_preflight(prompts_path)
    ...
    runner.package_template_invariant = package_template_invariant_0007

    def preflight_0007(prompts_path: Path) -> int:
        certificate = verify_tokenization_preflight(prompts_path)
        regression = verify_tokenization_verifier_regression()
    ...
    runner.preflight = preflight_0007

    def freeze_0007(prompts_path: Path) -> int:
        certificate = verify_tokenization_preflight(prompts_path)
        regression = verify_tokenization_verifier_regression()
    ...
    runner.freeze = freeze_0007

    def run_sequence_0007(
        prompt_id: str,
        token_ids: list[int],
        tokenizer: Any,
        weights: Any,
    ...
    runner.run_sequence = run_sequence_0007

    def execute_0007(prompts_path: Path, expected_freeze_sha256: str) -> int:
        certificate = verify_tokenization_preflight(prompts_path)
        verify_tokenization_verifier_regression()
        frozen = runner.read_json(runner.FREEZE)
        runner.require(
