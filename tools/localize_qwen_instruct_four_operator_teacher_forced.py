...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    require(
        ...
        == "qwen-instruct-four-operator-teacher-forced-causality-v1",
...
def step_comparison(reference: Tensor, candidate: Tensor, token_id: int) -> dict[str, Any]:
    ...
    candidate_token = int(top.indices[0])
    return {
...
def main() -> int:
    ...
    status = (
        "PASS_EXACT_FOUR_OPERATOR_TEACHER_FORCED_CAUSALITY_RECORDED"
        if all(checks.values())
        else "FAIL_CAUSALITY_PROBE_CHECK"
    )
    result = {
        "schema_version": 1,
        "classification": "qwen_instruct_option_b_four_operator_teacher_forced_causality",
        "status": status,
