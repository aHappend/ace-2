#!/usr/bin/env python3
"""Run one marker-free independent L2 review of the V8 package."""
...
MISSION_ID = "qwen25-full-finetune-v8-execution-package-l2-v3"
RULINGS = (
    "v8.full-parameter-distinct",
    "v8.fresh-successor-boundary",
    "v8.score-policy-schema",
    "v8.detached-worker-interlocks",
    "v8.marker-free-freeze",
    "v8.resource-claims-honest",
)
SOURCE_FILES = tuple(
    ROOT / "pilot/qwen25_05b_bf16_full_finetune_product_v8_runner" / name
    for name in (
        "runtime.py",
        "train.py",
        "evaluate_dev.py",
...
def verified_inputs() -> dict[str, Any]:
    ...
    require(not RUN_ROOT.exists(), "official V8 namespace exists")
    ...
    for name in ("v8-detached-launch-intent.json", "v8-detached-process.json", "v8-launch-terminal-status.json", "v8-worker-terminal-status.json"):
        require(not (OFFLINE / name).exists(), f"V8 execution record exists: {name}")
    ...
    require(self_test.get("status") == "PASS", "V8 self-test is not PASS")
    require(all(self_test.get("checks", {}).values()), "one or more V8 self-test checks failed")
    required_checks = {
    ...
    require(required_checks <= set(self_test.get("checks", {})), "V8 scorer/policy schema checks are incomplete")
    require(self_test.get("official_namespace_exists") is False, "V8 self-test reports a namespace")
    require(request.get("status") == "PENDING_INDEPENDENT_L2", "V8 review request status differs")
    require(request.get("official_namespace_exists") is False, "V8 review request reports a namespace")
    require(request.get("official_v7_checkpoint_or_output_dependency") is False, "V8 request depends on V7 execution evidence")
...
def check_existing() -> None:
    ...
    require(companion_matches(DECISION), "V8 L2 decision checksum differs")
    require(companion_matches(ACCEPTANCE), "V8 L2 acceptance checksum differs")
    decision = load(DECISION)
    acceptance = load(ACCEPTANCE)
    require(decision.get("reviewer_status") == "done", "V8 L2 decision is not done")
    require(acceptance.get("status") == "ACCEPTED_V8_FULL_FINETUNE_EXECUTION_PACKAGE_NO_ATTEMPT", "V8 L2 status differs")
    require(acceptance.get("accepted") is True, "V8 L2 acceptance is false")
    require(acceptance.get("attempt_authority_granted") is False, "V8 L2 grants attempt authority")
    require(acceptance.get("detached_launch_authorized") is False, "V8 L2 grants launch authority")
    require(acceptance.get("independent_from_constructor") is True, "V8 L2 is not independent")
    require(acceptance.get("execution_tree_sha256") == verified["tree_sha256"], "V8 L2 tree differs")
    require(acceptance.get("execution_manifest_sha256") == sha256(MANIFEST), "V8 L2 manifest differs")
    require(acceptance.get("contract_sha256") == sha256(CONTRACT), "V8 L2 contract differs")
    require(acceptance.get("reviewer_decision_sha256") == sha256(DECISION), "V8 L2 decision binding differs")


def main() -> int:
    if "--check" in sys.argv[1:]:
        check_existing()
        print("ACE2_FULL_FINETUNE_V8_EXECUTION_PACKAGE_L2_CHECK_PASS")
        return 0

    require(not DECISION.exists(), "V8 L2 decision already exists")
    require(not ACCEPTANCE.exists(), "V8 L2 acceptance already exists")
    verified = verified_inputs()
    ...
    objective = (
        "Independently review the exact marker-free ACE-2 Qwen2.5-0.5B V8 full-parameter "
        "BF16-compute execution package, including its fresh successor boundary, immutable pre-V7 "
        "source/data bindings, canonical evaluator schema, checkpoint selection, conservative "
        "resources, and detached-worker terminal interlocks."
    )
    instruction = (
        ...
        "Return done only if this is a fresh V8 identity with no V7 checkpoint, scored-row, namespace, "
        "authority, marker, or lifecycle dependency; the dev policy exactly matches the canonical V2 "
        "category_gates/minimum_hard_passes schema; all isolated synthetic gate paths reproduce; this "
        "is genuinely full-parameter training with no teacher or PEFT; every "
        "stateful stage authenticates the detached worker before state/evaluator access, the terminal "
        "status and no-relaunch design are credible, every recorded self-test check is reproducible, and resource "
        "numbers are labeled prerequisites rather than measurements. In the reason state exactly: "
        "'v8.full-parameter-distinct: supported', 'v8.fresh-successor-boundary: supported', "
        "'v8.score-policy-schema: supported', 'v8.detached-worker-interlocks: supported', "
        "'v8.marker-free-freeze: supported', and 'v8.resource-claims-honest: supported'. "
    ...
    review = Reviewer(runner, memory_maintenance_enabled=False).evaluate(
        ...
        round_max=1,
        session_id=sid,
        main_summary=f"Planner froze fresh marker-free V8 tree {verified['tree_sha256']} from immutable source/data inputs with no V7 execution-evidence dependency and recorded {verified['self_test_check_count']} passing semantic checks.",
        main_error=None,
        raw_evidence=json.dumps(verified, indent=2, sort_keys=True),
        scope="v8_full_finetune_exact_execution_package_marker_free_l2_only",
        checkpoint_path="",
    ...
    if status != "done":
        print(f"ACE2_FULL_FINETUNE_V8_EXECUTION_PACKAGE_L2_REPLAN status={status}")
        return 2
    acceptance = {
        "accepted": True,
        ...
        "schema_version": 1,
        "status": "ACCEPTED_V8_FULL_FINETUNE_EXECUTION_PACKAGE_NO_ATTEMPT",
    }
    write_json(ACCEPTANCE, acceptance)
    check_existing()
    print(f"ACE2_FULL_FINETUNE_V8_EXECUTION_PACKAGE_L2_PASS tree={verified['tree_sha256']}")
    return 0
