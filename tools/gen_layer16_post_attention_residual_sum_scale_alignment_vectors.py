#!/usr/bin/env python3
"""Generate focused vectors for the layer-16 post-attention residual sum."""
...
DEFAULT_OUTPUT = ROOT / "verification/generated/ace2_layer16_post_attention_residual_sum_scale_alignment"
HIDDEN = 896
...
def write_vectors(output: Path) -> dict[str, object]:
    ...
    residual = _read(group, "residual_s8", "i1", HIDDEN)
    residual_scale = _read(group, "residual_scale32_expanded", "<i8", HIDDEN)
    attention = _read(group, "attention_s8", "i1", HIDDEN)
    attention_scale = _read(group, "attention_scale32_expanded", "<i8", HIDDEN)
    output_group_scale = _read(
        group, "post_attention_output_group_scale32", "<i8", group_count
    )
    expected = _read(group, "post_attention_sum_s8", "i1", HIDDEN)
    saturation = _read(group, "post_attention_sum_saturation", "u1", HIDDEN)
    output_scale = np.repeat(output_group_scale, group_size)
    common_exponent = np.array(
        [
            min(_scale_exponent(residual_scale[index]),
                _scale_exponent(attention_scale[index]),
    ...
    _write_hex(output / "down_s8.hex", attention, 8)
    _write_hex(output / "down_scale32.hex", attention_scale, 32)
    ...
    (output / "ace2_layer16_post_attention_residual_sum_scale_alignment_constants.svh").write_text(
        "\n".join(
            [
    ...
    manifest = {
        ...
        "bindings": {
            ...
            "search_runner": _artifact(
                ROOT / "tools/ace2_checkpoint176_layer16_post_attention_residual_sum_scale_alignment_search.py"
            ),
        },
    }
...
def main() -> int:
    ...
    print(
        "ACE2_LAYER16_POST_ATTENTION_RESIDUAL_SUM_SCALE_ALIGNMENT_VECTOR_"
