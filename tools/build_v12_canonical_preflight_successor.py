#!/usr/bin/env python3
"""Build the immutable Base V12 canonical-byte read-only-preflight successor."""
...
V12_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v12_shellfree_action_root"
V8_STATIC_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_static_v8_action_root"
VERIFIER_SOURCE = ROOT / "tools/v12_canonical_preflight_verify_independent.py"
...
PACKAGE_PATH = V12_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_PACKAGE.json"
LAUNCHER_PATH = V12_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v12.py"
TRANSPORT_PATH = V12_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v12.py"
STATIC_VERIFIER_PATH = V12_ROOT / "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v12_shellfree.py"
REPORT_PATH = V12_ROOT / "V9_CANONICAL_BYTE_FAILURE_REPORT.md"
CANONICAL_PACKAGE = V12_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_PACKAGE.canonical.json"
CANONICAL_RESULT_SCHEMA = V12_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_RESULT_SCHEMA.canonical.json"
...
def replace_v9(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("V9", "V12").replace("v9", "v12").replace(
            "ace2:qk-gbfp8-base-v12:execute-once:2254dd91:20260813T2013Z",
            ACTION_ID,
        )
...
def main() -> int:
    require(not V12_ROOT.exists(), f"refusing to overwrite successor root: {V12_ROOT}")
    ...
    v12_schema_dir = V12_ROOT / "reference"
    schema_names = (
        "AUTHORITY_SCHEMA",
        "CREDENTIAL_SCHEMA",
    ...
    for name in schema_names:
        ...
        target = v12_schema_dir / f"QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_{name}.json"
        write_bytes(target, compact_bytes(replace_v9(strict_object(source.read_bytes()))))
        schema_paths[name] = target

    report = f"""# Base V9 canonical-byte preflight failure and V12 additive repair
...
"""
    ...
    launcher = v9_launcher.replace("V9", "V12").replace("v9", "v12")
    launcher = launcher.replace(
        'ACTION_ID = "ace2:qk-gbfp8-base-v12:execute-once:2254dd91:20260813T2013Z"',
...
'''
    ...
    transport = v9_transport.replace("V9", "V12").replace("v9", "v12")
    ...
    transport_argv = [
        ...
        str(PACKAGE_PATH),
        "--acceptance",
        str(V12_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"),
    ...
    package["static_file_policy"] = {
        "allowed_relative_files": [
            "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_PACKAGE.json",
            ...
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_AUTHORITY_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_CREDENTIAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_FIRST_TERMINAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V12_SHELLFREE_LEDGER_SCHEMA.json",
            "review/FRESH_L2_STATIC_ACCEPTANCE.json",
            "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v12.py",
            "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v12.py",
            "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v12_shellfree.py",
        ],
    ...
    for path in sorted(V12_ROOT.rglob("*"), reverse=True):
        if path.is_file():
            os.chmod(path, 0o444)
    for path in sorted((item for item in V12_ROOT.rglob("*") if item.is_dir()), reverse=True):
        os.chmod(path, 0o555)
    os.chmod(V12_ROOT, 0o555)
    print(json.dumps({
        "action_id": ACTION_ID,
        ...
        "root": str(V12_ROOT),
        "status": "BUILT_IMMUTABLE_V12_CANONICAL_PREFLIGHT_SUCCESSOR_NO_AUTHORITY",
    }, sort_keys=True))
    return 0
