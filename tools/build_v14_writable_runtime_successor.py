#!/usr/bin/env python3
"""Build the additive V14 static package with an external writable runtime.
...
V14_ROOT = PROJECT_ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v14_shellfree_action_root"
...
VERIFIER_SOURCE = PROJECT_ROOT / "tools/v14_writable_runtime_verify_independent.py"
...
def transform_text(text: str) -> str:
    return (
        ...
        .replace(str(V13_ROOT), str(V14_ROOT))
        .replace("V13", "V14")
        .replace("v13", "v14")
    )
...
def render_launcher(
    ...
    root_line = f'ROOT = Path("{V14_ROOT}")\n'
...
'''
    ...
    require(str(report_path) == str(V14_ROOT / "V13_RUNTIME_NAMESPACE_FAILURE_REPORT.md"), "report final path")
...
def _probe_create_once(parent: Path, label: str) -> None:
    probe = parent / f".v14-transport-{label}-create-once-probe"
    require(not os.path.lexists(probe), f"stale runtime probe: {probe}")
    _durable_create(probe, f"{ACTION_ID}:{label}\\n".encode("ascii"))
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    if hasattr(os, "O_NOFOLLOW"):
...
def main() -> int:
    require(not V14_ROOT.exists(), f"refusing to overwrite V14 package: {V14_ROOT}")
    require(not RUNTIME_ROOT.exists(), f"refusing to overwrite V14 runtime: {RUNTIME_ROOT}")
    require(V13_ROOT.is_dir(), "V13 package absent")
    require(VERIFIER_SOURCE.is_file(), "V14 independent verifier source absent")
    require(V13_FAILURE_RECORD.is_file(), "V13 failure record absent")
    require(INTERPRETER.is_file(), "frozen interpreter absent")

    package_stage = V14_ROOT.with_name(f".{V14_ROOT.name}.staging-{os.getpid()}")
    ...
    try:
        ...
        for name in schema_names:
            ...
            target = package_stage / f"reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_{name}.json"
            schema = transform_value(strict_object(source.read_bytes()))
            write_bytes(target, compact_bytes(schema))
            schema_paths[name] = target

        report = f"""# V13 runtime-namespace failure and V14 additive successor
...
The additive V14 action is `{ACTION_ID}`.  Its package root is immutable and
...
"""
        ...
        expected_paths = [
            ...
            ("package", V14_ROOT / canonical_package.relative_to(package_stage)),
            ("result_schema", V14_ROOT / canonical_schema.relative_to(package_stage)),
        ...
        launcher_stage = package_stage / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v14.py"
        transport_stage = package_stage / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v14.py"
        verifier_stage = package_stage / "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v14_shellfree.py"
        write_bytes(
            launcher_stage,
            render_launcher(
                package_stage,
                expected_paths,
                hash_sources,
                V14_ROOT / report_stage.relative_to(package_stage),
            ),
        )
        write_bytes(transport_stage, render_transport())
        write_bytes(verifier_stage, VERIFIER_SOURCE.read_bytes())

        package_path_stage = package_stage / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_PACKAGE.json"
        ...
        local_paths = [
            ("authority_schema", V14_ROOT / schema_paths["AUTHORITY_SCHEMA"].relative_to(package_stage)),
            ("credential_schema", V14_ROOT / schema_paths["CREDENTIAL_SCHEMA"].relative_to(package_stage)),
            ("ledger_schema", V14_ROOT / schema_paths["LEDGER_SCHEMA"].relative_to(package_stage)),
            ("first_terminal_schema", V14_ROOT / schema_paths["FIRST_TERMINAL_SCHEMA"].relative_to(package_stage)),
            ("fresh_l2_acceptance_schema", V14_ROOT / schema_paths["FRESH_L2_ACCEPTANCE_SCHEMA"].relative_to(package_stage)),
            ("transport", V14_ROOT / transport_stage.relative_to(package_stage)),
            ("launcher", V14_ROOT / launcher_stage.relative_to(package_stage)),
            ("static_verifier", V14_ROOT / verifier_stage.relative_to(package_stage)),
            ("forensic_report", V14_ROOT / report_stage.relative_to(package_stage)),
        ]
        stage_lookup = {
            V14_ROOT / path.relative_to(package_stage): path
        ...
        package["accepted_byte_derivation"] = {
            "package": {
                ...
                "canonical_path": str(V14_ROOT / canonical_package.relative_to(package_stage)),
            ...
            "result_schema": {
                ...
                "canonical_path": str(V14_ROOT / canonical_schema.relative_to(package_stage)),
        ...
        transport_final = V14_ROOT / transport_stage.relative_to(package_stage)
        launcher_final = V14_ROOT / launcher_stage.relative_to(package_stage)
        package_final = V14_ROOT / package_path_stage.relative_to(package_stage)
        acceptance_final = V14_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"
        ...
        package["production_canonical_preflight_proof"] = {
            ...
            "independent_verifier": str(V14_ROOT / verifier_stage.relative_to(package_stage)),
        ...
        package["static_file_policy"] = {
            "allowed_relative_files": [
                "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_PACKAGE.json",
                ...
                "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_AUTHORITY_SCHEMA.json",
                "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_CREDENTIAL_SCHEMA.json",
                "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_FIRST_TERMINAL_SCHEMA.json",
                "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json",
                "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_LEDGER_SCHEMA.json",
                "review/FRESH_L2_STATIC_ACCEPTANCE.json",
                "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v14.py",
                "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v14.py",
                "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v14_shellfree.py",
            ],
        ...
        package["root_id"] = "qk_gbfp8_head64_granularity_sweep_execution_v14_shellfree_action_root"
        for record_name, record in package["record_formats"].items():
            schema_id = record["schema_id"]
            schema_path = Path(record["schema_path"])
            stage_schema = package_stage / schema_path.relative_to(V14_ROOT)
        ...
        os.rename(package_stage, V14_ROOT)
    except Exception:
    ...
    print(compact_bytes({
        "action_id": ACTION_ID,
        "manifest_raw_sha256": sha256_file(V14_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V14_SHELLFREE_PACKAGE.json"),
        "package_root": str(V14_ROOT),
        "runtime_root": str(RUNTIME_ROOT),
        "status": "BUILT_IMMUTABLE_V14_WRITABLE_RUNTIME_SUCCESSOR_NO_AUTHORITY",
    }).decode("ascii"), end="")
    return 0
