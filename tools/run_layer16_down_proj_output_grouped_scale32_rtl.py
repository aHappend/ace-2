#!/usr/bin/env python3
"""Run focused RTL verification for grouped layer-16 down-projection output."""
...
GENERATED = ROOT / "verification/generated/ace2_layer16_down_proj_output_grouped_scale32"
RTL = ROOT / "rtl/ace2_layer16_down_proj_output_grouped_scale32_core.sv"
TB = ROOT / "verification/tb/ace2_layer16_down_proj_output_grouped_scale32_tb.sv"
SEARCH = ROOT / "tools/ace2_checkpoint176_layer16_down_proj_output_grouped_scale32_search.py"
GENERATOR = ROOT / "tools/gen_layer16_down_proj_output_grouped_scale32_vectors.py"
UNIT_TEST = ROOT / "verification/test_layer16_down_proj_output_grouped_scale32_reference.py"
...
PASS_PATTERN = re.compile(
    r"ACE2_LAYER16_DOWN_PROJ_OUTPUT_GROUPED_SCALE32_RTL_PASS "
...
def main() -> int:
    ...
    if selected_as_repair:
        require(
            bool(candidate_result["first_token_restored"])
            or candidate_result["selected_reference_rank"] < candidate_result["baseline_reference_rank"],
            "selected candidate does not satisfy the frozen score policy",
        )
    else:
        require(
    ...
    run(
        [
            ...
            "-m",
            "unittest",
            "verification.test_layer16_down_proj_output_grouped_scale32_reference",
            "-v",
        ],
        unit_log,
    )
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-down-output-grouped-", dir=ROOT / "build") as temporary:
        ...
        compile_stdout = run(
            [
                ...
                "-s",
                "ace2_layer16_down_proj_output_grouped_scale32_tb",
                "-o",
                str(executable),
                str(RTL),
                str(TB),
    ...
    validation_rank = (
        ...
        if selected_as_repair
        else candidate_result["best_observed_reference_rank"]
    )
    validation_top = (
        candidate_result["selected_top_token_id"]
        if selected_as_repair
        else candidate_result["best_observed_top_token_id"]
    )
    generated_members = sorted(path for path in GENERATED.iterdir() if path.is_file())
    result = {
        "schema_version": 1,
        "generated_at_utc": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
        "status": "PASS_FOCUSED_LAYER16_DOWN_PROJ_OUTPUT_GROUPED_SCALE32_RTL_REFERENCE_AGREEMENT",
        ...
        "claim_boundary": (
            "Frozen candidate plus focused synthesizable boundary RTL/reference agreement "
            "only; independent Fresh-L2 acceptance remains required."
        ),
        "numeric_contract": {
            "input_samples": "signed A8",
            "input_scale": "one explicit producer Scale32",
            "output_samples": "signed A8",
            "output_scales": f"one explicit model-derived Scale32 per {group_size} samples",
    ...
    print(
        "ACE2_LAYER16_DOWN_PROJ_OUTPUT_GROUPED_SCALE32_VERIFY_PASS "
        f"group={group_size} rank={validation_rank} outputs={metrics['outputs']} "
        f"selected={int(selected_as_repair)} xz_clean=1 warning_clean=1"
    )
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"ACE2_LAYER16_DOWN_PROJ_OUTPUT_GROUPED_SCALE32_VERIFY_FAIL detail={error}",
            file=sys.stderr,
            flush=True,
        )
        raise
