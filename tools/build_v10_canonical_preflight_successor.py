#!/usr/bin/env python3
"""Build the immutable Base V10 canonical-byte read-only-preflight successor."""
...
V10_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree_action_root"
V8_STATIC_ROOT = ROOT / "reference/qk_gbfp8_head64_granularity_sweep_static_v8_action_root"
VERIFIER_SOURCE = ROOT / "tools/v10_canonical_preflight_verify_independent.py"
...
PACKAGE_PATH = V10_ROOT / "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_PACKAGE.json"
LAUNCHER_PATH = V10_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v10.py"
TRANSPORT_PATH = V10_ROOT / "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v10.py"
STATIC_VERIFIER_PATH = V10_ROOT / "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree.py"
REPORT_PATH = V10_ROOT / "V9_CANONICAL_BYTE_FAILURE_REPORT.md"
CANONICAL_PACKAGE = V10_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_PACKAGE.canonical.json"
CANONICAL_RESULT_SCHEMA = V10_ROOT / "accepted/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_STATIC_V8_RESULT_SCHEMA.canonical.json"
...
def replace_v9(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("V9", "V10").replace("v9", "v10").replace(
            "ace2:qk-gbfp8-base-v10:execute-once:2254dd91:20260813T2013Z",
            ACTION_ID,
        )
...
def main() -> int:
    require(not V10_ROOT.exists(), f"refusing to overwrite successor root: {V10_ROOT}")
    ...
    v10_schema_dir = V10_ROOT / "reference"
    schema_names = (
        "AUTHORITY_SCHEMA",
        "CREDENTIAL_SCHEMA",
    ...
    for name in schema_names:
        ...
        target = v10_schema_dir / f"QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_{name}.json"
        write_bytes(target, compact_bytes(replace_v9(strict_object(source.read_bytes()))))
        schema_paths[name] = target

    report = f"""# Base V9 canonical-byte preflight failure and V10 additive repair
...
"""
    ...
    launcher = v9_launcher.replace("V9", "V10").replace("v9", "v10")
    launcher = launcher.replace(
        'ACTION_ID = "ace2:qk-gbfp8-base-v10:execute-once:2254dd91:20260813T2013Z"',
...
'''
    ...
    transport = v9_transport.replace("V9", "V10").replace("v9", "v10")
    ...
    transport_argv = [
        ...
        str(PACKAGE_PATH),
        "--acceptance",
        str(V10_ROOT / "review/FRESH_L2_STATIC_ACCEPTANCE.json"),
    ...
    package["static_file_policy"] = {
        "allowed_relative_files": [
            "QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_PACKAGE.json",
            ...
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_AUTHORITY_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_CREDENTIAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_FIRST_TERMINAL_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_FRESH_L2_ACCEPTANCE_SCHEMA.json",
            "reference/QK_GBFP8_HEAD64_GRANULARITY_SWEEP_EXECUTION_V10_SHELLFREE_LEDGER_SCHEMA.json",
            "review/FRESH_L2_STATIC_ACCEPTANCE.json",
            "tools/qk_gbfp8_head64_granularity_sweep_execute_once_shellfree_v10.py",
            "tools/qk_gbfp8_head64_granularity_sweep_transport_shellfree_v10.py",
            "tools/verify_qk_gbfp8_head64_granularity_sweep_execution_v10_shellfree.py",
        ],
    ...
    for path in sorted(V10_ROOT.rglob("*"), reverse=True):
        if path.is_file():
            os.chmod(path, 0o444)
    for path in sorted((item for item in V10_ROOT.rglob("*") if item.is_dir()), reverse=True):
        os.chmod(path, 0o555)
    os.chmod(V10_ROOT, 0o555)
    print(json.dumps({
        "action_id": ACTION_ID,
        ...
        "root": str(V10_ROOT),
        "status": "BUILT_IMMUTABLE_V10_CANONICAL_PREFLIGHT_SUCCESSOR_NO_AUTHORITY",
    }, sort_keys=True))
    return 0
