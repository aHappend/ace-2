#!/usr/bin/env python3
"""Run focused RTL verification for the layer-16 post-attention residual sum."""
...
GENERATED = ROOT / "verification/generated/ace2_layer16_post_attention_residual_sum_scale_alignment"
...
TB = ROOT / "verification/tb/ace2_layer16_post_attention_residual_sum_scale_alignment_tb.sv"
SEARCH = ROOT / "tools/ace2_checkpoint176_layer16_post_attention_residual_sum_scale_alignment_search.py"
GENERATOR = ROOT / "tools/gen_layer16_post_attention_residual_sum_scale_alignment_vectors.py"
UNIT_TEST = ROOT / "verification/test_layer16_post_attention_residual_sum_scale_alignment_reference.py"
RUNNER = Path(__file__).resolve()
RTL_TOP = "ace2_layer16_post_attention_residual_sum_scale_alignment_core"


PASS_PATTERN = re.compile(
    r"ACE2_LAYER16_POST_ATTENTION_RESIDUAL_SUM_SCALE_ALIGNMENT_RTL_PASS "
...
def main() -> int:
    ...
    selected = candidate_result["selected_output_group_size"] is not None
    require(
        group_size
        == (
            candidate_result["selected_output_group_size"]
            if selected
            else candidate_result["best_non_control_output_group_size"]
        ),
        "RTL validation contract differs from selected/best-null result",
    )
    ...
    run(
        [
            ...
            "-m",
            "unittest",
            "verification.test_layer16_post_attention_residual_sum_scale_alignment_reference",
            "-v",
        ],
        unit_log,
    )
    ...
    with tempfile.TemporaryDirectory(prefix="ace2-layer16-residual-sum-", dir=ROOT / "build") as temporary:
        ...
        compile_stdout = run(
            [
                ...
                "-s",
                "ace2_layer16_post_attention_residual_sum_scale_alignment_tb",
                "-o",
                str(executable),
                str(ARITHMETIC_RTL),
                str(RTL),
    ...
    result = {
        ...
        "status": "PASS_FOCUSED_LAYER16_POST_ATTENTION_RESIDUAL_SUM_SCALE_ALIGNMENT_SELECTED_OR_BEST_NULL_RTL_REFERENCE_AGREEMENT",
        ...
        "validation_candidate": {
            ...
            "reference_rank": (
                ...
                if selected
                else candidate_result["best_non_control_reference_rank"]
            ),
            "baseline_reference_rank": candidate_result["baseline_reference_rank"],
            "top_token_id": (
                candidate_result["selected_top_token_id"]
                if selected
                else candidate_result["best_non_control_top_token_id"]
            ),
        ...
        "claim_boundary": (
            "Frozen candidate plus focused synthesizable boundary RTL/reference agreement only; "
        ...
        "numeric_contract": {
            ...
            "source_scales": "independent explicit Scale32 residual and self-attention-output metadata",
        ...
        "source_and_constraint_binding": {
            ...
            "arithmetic_rtl_sha256": sha256(ARITHMETIC_RTL),
    ...
    print(
        "ACE2_LAYER16_POST_ATTENTION_RESIDUAL_SUM_SCALE_ALIGNMENT_VERIFY_PASS "
        f"group={group_size} rank={result['validation_candidate']['reference_rank']} "
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"ACE2_LAYER16_POST_ATTENTION_RESIDUAL_SUM_SCALE_ALIGNMENT_VERIFY_FAIL detail={error}",
            file=sys.stderr,
            flush=True,
        )
        raise
