#!/usr/bin/env python3
"""Phase-safe fixed-input-scale launcher for nonofficial hybrid attempt 0020."""
...
PROMPTS = PRIVATE / "prompts-0020.json"
SELECTION = PRIVATE / "candidate-selection-0020.json"
TOKENIZATION_PREFLIGHT = PRIVATE / "exact-tokenization-preflight-0020.json"
TOKENIZATION_REGRESSION = PRIVATE / "tokenization-verifier-regression-0020.json"
INPUT_SCALE_REGRESSION = PRIVATE / "fixed-input-scale-regression-0020.json"
SCALE_REGRESSION = PRIVATE / "retained-scale-regression-0020.json"
LIFECYCLE_REGRESSION = PRIVATE / "record-lifecycle-regression-0020.json"
GATE_ORDER_REGRESSION = PRIVATE / "gate-order-regression-0020.json"
TEMPLATE_GATE = PRIVATE / "package-template-invariant-0020.json"
DEPENDENCY_PROBE = PRIVATE / "dependency-probe-0020.json"
LAUNCH_REGRESSION = PRIVATE / "launch-fidelity-regression-0020.json"
ARGV_SHAPE_REGRESSION = PRIVATE / "argv-shape-regression-0020.json"
CLOSURE_CERTIFICATE = PRIVATE / "closure-certificate-0020.json"
PHASE_TRANSITION_REGRESSION = PRIVATE / "phase-transition-regression-0020.json"
INVOCATION_REGRESSION = PRIVATE / "invocation-fidelity-regression-0020.json"
...
CANDIDATES = (
    {"id": "fresh-0020-a", "text": "Why do cuttlefish flash patterns?"},
    {"id": "fresh-0020-b", "text": "How do puffballs release spores?"},
    {"id": "fresh-0020-c", "text": "Where do sandgrouse gather water?"},
    {"id": "fresh-0020-d", "text": "Why does pumice float briefly?"},
    {"id": "fresh-0020-e", "text": "How do fiddleheads unfurl?"},
    {"id": "fresh-0020-f", "text": "What makes ice needles rise?"},
)
...
def _invocation_regression_record(
    ...
    return {
        "schema_version": 1,
        "mission_id": "stage1rank1hybrid20execute",
        "attempt_identity": "nonofficial-hybrid-0020",
...
def _verify_invocation_regression() -> dict[str, Any]:
    _preloader_require(
        ...
        "0020 invocation-fidelity regression is absent or not regular",
    )
    ...
    _preloader_require(
        ...
        and record.get("mission_id") == "stage1rank1hybrid20execute"
        and record.get("attempt_identity") == "nonofficial-hybrid-0020"
        ...
        "0020 invocation-fidelity regression changed or is incomplete",
    )
    return record


def _verify_0019_stdlib_seal() -> None:
    terminal = (
        ROOT
        / "evidence/verification/stage1-layer23-v-rank1-hybrid-v1/"
        "nonofficial-hybrid-0019/terminal-record.json"
    )
    launcher = ROOT / "tools/run_stage1_layer23_v_rank1_hybrid_0019.py"
    for path in (terminal, launcher):
        _preloader_require(
            stat.S_ISREG(os.lstat(path).st_mode),
            f"sealed 0019 artifact is not regular: {path}",
        )
    terminal_record = json.loads(terminal.read_text(encoding="utf-8"))
    _preloader_require(
        _sha256_file(terminal) == SEALED_0019_TERMINAL_SHA256
        and _sha256_file(launcher) == SEALED_0019_LAUNCHER_SHA256
...
def _run_invocation_regression(inherited_snapshot: Path) -> int:
    ...
    _preloader_require(
        ...
        "0020 invocation or execution namespace already exists",
    )
    _verify_0019_stdlib_seal()
    record = _invocation_regression_record(
        _read_environment_snapshot(inherited_snapshot)
    )
    _preloader_require(
        all(record["checks"].values()),
        "0020 invocation-fidelity regression failed",
    )
    ...
    print(
        "ACE2_HYBRID_0020_INVOCATION_FIDELITY_REGRESSION_PASS "
...
def _seal_preloader_failure(error: BaseException) -> None:
    ...
    record = {
        ...
        "failure": {
            ...
            "regression": "No retry, replay, or resume; preserve all 0020 artifacts.",
        },
    }
    try:
...
def _configure_runner(
    ...
    runner.MISSION_ID = "stage1rank1hybrid20execute"
    runner.ATTEMPT_ID = "nonofficial-hybrid-0020"
    ...
    def verify_predecessor_seals_0020() -> None:
        prior_verify_predecessor_seals()
        verify_0017_seal()
        verify_0018_seal()
        verify_0019_seal()

    runner.verify_predecessor_seals = verify_predecessor_seals_0020

    def verify_exact_interpreter_0020() -> None:
        prior_verify_exact_interpreter()
        _verify_invocation_regression()

    runner.verify_exact_interpreter = verify_exact_interpreter_0020
    ...
    def verify_argv_shape_regression() -> dict[str, Any]:
        runner.require(
            ...
            "0020 argv-shape regression is absent or not regular",
        )
        record = runner.read_json(ARGV_SHAPE_REGRESSION)
        runner.require(
            record == expected_argv_shape_regression()
            and all(record["checks"].values()),
            "0020 argv-shape regression changed",
        )
        return record
    ...
    def binding_core() -> dict[str, Any]:
        return {
            ...
            "sealed_predecessor": verify_0019_seal(),
    ...
    def verify_closure_certificate() -> dict[str, Any]:
        runner.require(
            ...
            "0020 closure certificate is absent or not regular",
        )
        ...
        runner.require(
            ...
            "0020 immutable closure certificate changed",
        )
        return certificate
    ...
    def verify_phase_transition_regression() -> dict[str, Any]:
        runner.require(
            ...
            "0020 phase-transition regression is absent",
        )
        ...
        runner.require(
            ...
            and all(record["checks"].values()),
            "0020 phase-transition regression changed",
        )
        return record
    ...
    def phase_binding_regression() -> int:
        ...
        print(
            "ACE2_HYBRID_0020_PHASE_BINDING_REGRESSION_PASS "
    ...
    runner.source_records = source_records_0020

    def execution_bound_paths_0020(
        prompts_path: Path,
        model: Path,
        tokenizer_json: Path,
        tokenizer_config: Path,
    ...
    runner.execution_bound_paths = execution_bound_paths_0020

    def package_bindings_0020(*args: Any, **kwargs: Any) -> dict[str, Any]:
        bindings = prior_package_bindings(*args, **kwargs)
        bindings["phase_binding"] = phase_binding_manifest()
        return bindings

    runner.package_bindings = package_bindings_0020

    def package_template_0020(prompts_path: Path) -> int:
        verify_phase_transition_regression()
    ...
    runner.package_template_invariant = package_template_0020

    def preflight_0020(prompts_path: Path) -> int:
        verify_phase_transition_regression()
    ...
    runner.preflight = preflight_0020

    def freeze_0020(prompts_path: Path) -> int:
        certificate = verify_closure_certificate()
        verify_phase_transition_regression()
    ...
    def package_0020(prompts_path: Path) -> int:
    ...
    runner.package_execution = package_0020

    def authority_0020(prompts_path: Path) -> int:
        verify_frozen_phase_binding()
    ...
    runner.grant_authority = authority_0020
    ...
    def child_entry_attestation_0020(
        prompts_path: Path,
        ...
        runner.require(
            Path(PRE_IMPORT_ARGV[0]).resolve() == LAUNCHER,
            "pre-import argv[0] does not resolve to the exact 0020 launcher",
        )
        runner.require(
    ...
    runner.child_entry_attestation = child_entry_attestation_0020

    def execute_0020(prompts_path: Path, expected_freeze_sha256: str) -> int:
        child_entry_attestation_0020(prompts_path, expected_freeze_sha256)
        return prior_execute(prompts_path, expected_freeze_sha256)

    runner.execute = execute_0020
    ...
    def verify_0020() -> int:
        ...
        runner.require(
            ...
            is True,
            "0020 terminal argv, phase-safe, or RTL-in-loop evidence changed",
        )
        print(
            "ACE2_HYBRID_0020_DECISIVE_VERIFY_PASS "
...
def main() -> int:
    try:
        ...
        _preloader_require(
            ...
            "0020 invocation map differs from the frozen 0010 environment",
        )
