#!/usr/bin/env python3
"""Run the non-consuming blinded V3 train/dev/holdout prompt separation scan."""
...
MANIFEST = (
    ROOT
    / "research/training/qwen2.5-0.5b-instruct-bf16-lora-product-v3"
...
SIDECAR = (
    ...
    / "qwen25-05b-instruct-bf16-lora-product-v3-cross-split-blinded-sidecar.json"
)
ATTESTATION = (
    ROOT
    / "research/raw/specification"
    / "qwen25-05b-instruct-bf16-lora-product-v3-cross-split-blinded-attestation.json"
)
OPERATOR_APPROVAL_REFERENCE = (
    "LIVE MANAGER / OPERATOR DIRECTIVES — authoritative post-diagnosis routing "
    "2026-08-07T23:10Z"
)
...
def main() -> None:
    ...
    leakage = contract["leakage_contract"]
    policy = manifest["leakage_policy"]
    if not leakage["prompt_only_blinded_scanner"] or not policy["prompt_only_blinded_scanner"]:
        raise RuntimeError("frozen blinded method differs")
    expected_hashes = {
        "dev_sha256": leakage["dev_sha256"],
        "holdout_sha256": leakage["holdout_sha256"],
        "train_sha256": manifest["files"]["train.jsonl"]["sha256"],
    }
    source_paths = {"dev": DEV, "holdout": HOLDOUT, "train": TRAIN}
    source_counts = {"dev": 56, "holdout": 56, "train": 288}
    ...
    threshold = float(
        leakage["maximum_train_or_probe_to_evaluation_five_gram_jaccard"]
    )
    ...
    attestation = {
        ...
        "producer": {
            ...
            "role": "constructor_blinded_split_auditor",
