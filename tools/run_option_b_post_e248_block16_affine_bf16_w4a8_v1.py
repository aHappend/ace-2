#!/usr/bin/env python3
"""Execute exactly one frozen block16-affine BF16 W4A8 reference campaign."""
...
CANDIDATE_ID = "option_b_post_e248_block16_affine_bf16_offset_step_w4a8_v1"
MISSION_LABEL = "post-e248-block16-affine-bf16-w4a8-v1"
TASK_ID = "job-e248-b16bf16-v1"
...
SELF_TEST_OUTPUT = ROOT / "build/post-e248-block16-affine-bf16-w4a8-v1-self-test.json"
PREFLIGHT_OUTPUT = ROOT / "build/post-e248-block16-affine-bf16-w4a8-v1-preflight.json"
PREEXECUTION_CONTRACT = ROOT / "build/post-e248-block16-affine-bf16-w4a8-v1-preexecution-contract.json"
...
PREDECESSOR_REVIEW = ROOT / "research/raw/specification/e248e307675a-block32-affine-review-replan-20260807T142652Z.json"
...
EXPECTED_BLOCK_COUNT = 61_745_152
...
PLAN_SHA256 = "fdb395d1690cd2ffb6fe88acf0ea9a91cdfeaf376bb3be21d5aa10cb8870ac95"
TASK_SHA256 = "a989549146aaac8c79c019214e635b93fd166522d2f1d66f6fad5c845d1ee9bf"
...
PREDECESSOR_REVIEW_SHA256 = "20d49d99e3eda12ab672cc0340bb461471fe9e6ff3c6fa1a174cbd7e95805820"
...
def build_candidate(label: str) -> tuple[nn.Module, dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    try:
        for ordinal, (name, module) in enumerate(modules, start=1):
            started = time.monotonic()
            print(
                f"V19_BLOCK16_BF16_CONSTRUCT_{label.upper()} {ordinal}/{len(modules)} {name}",
...
def source_weight_reproduction() -> dict[str, Any]:
    ...
    with torch.no_grad():
        for ordinal, (name, module) in enumerate(modules, start=1):
            print(f"V19_BLOCK16_BF16_SOURCE_WEIGHT_PREFLIGHT {ordinal}/{len(modules)} {name}", flush=True)
...
def assemble_preexecution_contract(source: dict[str, Any], versions: dict[str, Any]) -> dict[str, Any]:
    bindings = {
    ...
    return {
        ...
        "marker_boundaries": {
            "construction_started": "after exclusive official-root creation and before full-model construction",
        ...
        "claim_boundary": "Frozen software-reference execution inputs and exactly-once boundaries only; no candidate text, quality conclusion, RTL, PPA, U280, or stage-advance claim.",
    }
...
def preflight() -> dict[str, Any]:
    ...
    require(plan["fresh_authority"]["task_id"] == TASK_ID, "plan task identity differs")
    weight_reproduction = source_weight_reproduction()
    preexecution = assemble_preexecution_contract(source, versions)
    ...
    return {
        ...
        "claim_boundary": "Non-consuming restoration, source-weight, codec, parser, matrix, evaluator, guard, manifest, marker, namespace, and binding checks only; no candidate text generation.",
    }
...
def translated_print(*args: Any, **kwargs: Any) -> None:
    if args and isinstance(args[0], str):
        args = (args[0].replace("E248E307675A", "V19_BLOCK16_BF16"), *args[1:])
