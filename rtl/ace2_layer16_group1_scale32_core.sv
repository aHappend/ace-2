...
// Exact one-lane Scale32 conversion and aligned sum for the selected
...
module ace2_group1_scale32_quant_sum_core (
    input  logic                       clk_i,
    input  logic                       rst_ni,
    input  logic                       clear_i,
    ...
    input  logic signed [1*8-1:0]      attention_source_s8_i,
    input  logic signed [1*8-1:0]      down_source_s8_i,
    ...
    output logic signed [1*8-1:0]      attention_group_s8_o,
    output logic signed [1*8-1:0]      down_group_s8_o,
    output logic signed [1*8-1:0]      sum_s8_o,
    output logic        [0:0]          attention_saturation_o,
    output logic        [0:0]          down_saturation_o,
    output logic        [0:0]          sum_saturation_o,
    output logic                       descriptor_error_o,
    output logic                       numeric_overflow_o
);
    logic out_valid_q;
    logic signed [7:0] attention_group_q;
    logic signed [7:0] down_group_q;
    logic signed [7:0] sum_q;
    logic [0:0] attention_saturation_q;
    logic [0:0] down_saturation_q;
    logic [0:0] sum_saturation_q;
    logic descriptor_error_q;
    logic numeric_overflow_q;

    logic signed [7:0] attention_group_w;
    logic signed [7:0] down_group_w;
    logic signed [7:0] sum_w;
    logic [0:0] attention_saturation_w;
    logic [0:0] down_saturation_w;
    logic [0:0] sum_saturation_w;
    ...
    always @* begin : quantize_and_sum_comb
        ...
        for (lane = 0; lane < 1; lane = lane + 1) begin
...
module ace2_layer16_group1_rmsnorm_q_core #(
    ...
    function automatic logic [2:0] popcount1(input logic [0:0] value);
        begin
            popcount1 = {2'd0, value[0]};
        end
    endfunction
    ...
    ace2_group1_scale32_quant_sum_core quant_sum_unit (
        .clk_i,
        .rst_ni,
        .clear_i,
        ...
        if (!rst_ni) begin
            ...
            if (quant_out_valid && quant_out_ready) begin
                ...
                attention_saturation_count_q <= attention_saturation_count_q +
                    {9'd0, popcount1(quant_attention_saturation)};
                down_saturation_count_q <= down_saturation_count_q +
                    {9'd0, popcount1(quant_down_saturation)};
                sum_saturation_count_q <= sum_saturation_count_q +
                    {9'd0, popcount1(quant_sum_saturation)};
